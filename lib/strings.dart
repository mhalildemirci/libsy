class AppStrings {
  static const Map<String, Map<String, String>> imageDescriptions = {
    'en': {
      '2 Poems.jpg':
          'Oscar Wilde was a 19th-century Irish writer, playwright and poet.He is best known forhis plays and satirical works, but he also left important works as a poet.Wildes poetry is often noted for its aesthetic and emotional depth and is often rendered with irony and wit.',
      'A Christmas Carol.jpg':
          'The story begins with Scrooge being visited on Christmas Eve by the ghost of his old partner Jacob Marley. Marley comes to warn Scrooge of the consequences of his life of selfishness and lovelessness and convinces Scrooge to go on a journey visited by three ghosts.',
      '11 Intentions.jpg':
          'Oscar Wilde was a 19th-century Irish writer, playwright and poet.He is best known forhis plays and satirical works, but he also left important works as a poet.Wildes poetry is often noted for its aesthetic and emotional depth and is often rendered with irony and wit.',
      'A London Life.jpg':
          'The plot revolves around a crumbling marriage and its impact on many people, especially Laura Wing, the sister of his soon-to-be divorced wife.',
      'A Modern Utopia.jpg':
          'The novel is best known for the idea that a volunteer class of nobles, known as Samurai, could effectively govern a "kinetic, not static" world state[2] and thus "solve the problem of combining progress with political stability".',
      'a plug year.jpg':
          'It was written by Daniel Defoe in the form of a diary describing what happened during the great plague epidemic in London. The spread of the epidemic in the city, the difficulties people faced, the ways the disease spread and the reactions of the society to this tragedy are discussed in detail in the work.',
      'america.jpg':
          'The story begins with a young man, Karl Rossmann, being sent to America. Karl is exiled from Europe because of a shameful incident that shamed his family and he is forced to go to America.',
      'anna karenina.jpg':
          "It tells the story of Anna's unhappy marriage and her forbidden love affair with the young and charismatic officer Count Vronsky. Anna's love and social ostracism lead to her mental breakdown and a tragic ending.",
      'art of war.jpg':
          'It is a classic text written by the Chinese military strategist Sun Tzu. Dating back to the 5th century BC, it contains in-depth insights on military strategy and leadership.',
      'ayesha the return of she.jpg':
          'Ayesha, The Return of She is a gothic-fantasy novel by English Victorian writer H. Rider Haggard, published in 1905 as a sequel to She. Chronologically, it is the last novel in the Ayesha and Allan Quatermain series.',
      'beyond good and evil.jpg':
          'Beyond Good and Evil: An Introduction to a Future Philosophy is a book written by the German philosopher Friedrich Nietzsche, published in 1886.',
      'cadi.jpg':
          "he widower Naşit Nefi Efendi's has remarried and witches have begun to roam his mansion. Naşit Nefi Efendi's first wife, Mrs. Binnaz, returns as a witch, haunting Naşit Nefi Efendi and his wives and giving them no rest.",
      'boon.jpg':
          'is about a group of intellectuals and thinkers discussing the progress and evolutionary journey of humanity.',
      'ask batagi.jpg':
          'It is the story of the marriage of Naki Efendi, who has been married twice, who is bored with the buttermilk-hearted women who are spoiled by his family, with his third wife Bedia. A novel that reveals how corrupt the old Istanbul life, loves and relationships are.',
      'Anticipations.jpg':
          'In "Anticipations", Wells discusses the social and technological developments of the early 20th century, political changes and how they might shape the future of humanity. ',
      'aski memnu.jpg':
          'Bihter was married off to the much older Adnan Bey, but later began a forbidden relationship with Behlül, with whom she fell in love. On the other hand, Mr. Adnan is niece Nihal has a platonic love for Behlül.',
      'bealby.jpg':
          'Bealby, the protagonist of the novel, escapes to save himself from an unwanted family environment and finds a job in a hotel. However, here too, he finds himself involved in a series of events and embarks on unexpected adventures. ',
      'kipps.jpg':
          'Kipps, a tailor apprentice in a small town, suddenly becomes a rich man with an inherited fortune. Thanks to this fortune, he rises to higher social classes and meets various segments of English society.',
      'Les Miserables.jpg':
          "Valjean is unjustly sentenced to nineteen years in prison, after which he embarks on a moral journey as he tries to reintegrate into society. Alongside him are other characters, such as the police officer Javert.",
      'meditations.jpg':
          "Marcus Aurelius, as a ruler of the Roman Empire, faced the challenges of everyday life, while remaining firmly rooted in the teachings of Stoic philosophy. this book reflects his inner thoughts and spiritual struggles, while also addressing universal issues such as human nature, justice, virtue and destiny.",
      'metamorphosis.jpg':
          'The novel begins with an unexpected transformation of a man named Gregor Samsa. One morning, Gregor wakes up to find himself transformed into an insect. This transformation begins a process of alienation and isolation, both physical and psychological.',
      'Middlemarch.jpg':
          'The story begins with the dreams of Dorothea Brooke, a young and idealistic woman, about marriage. Dorothea marries Mr. Casaubon, a wealthy but elderly clergyman, only to find a life that is the opposite of her dreams of marriage.',
      'pickwick club.jpg':
          'The novel follows the adventures of the Pickwick Club, a group of individuals who come together under the leadership of a rich and cheerful man named Samuel Pickwick. ',
      'othello.jpg':
          '"Othello," is a famous tragedy play written by William Shakespeare. The play deals with themes such as jealousy, betrayal and racism and dramatizes the tragic fall of Othello, a soldier.',
      'odysey.jpg':
          "'The Odyssey' deals with universal themes such as loyalty, love, courage, honor, cunning and justice. In addition, Homer's epic offers a rich portrait of ancient Greek mythology, customs and values.",
      'peter pan.jpg':
          "'Peter Pan,' It is a famous children's book written by J.M. Barrie. The story tells the fantastic adventures of a boy named Peter Pan and three siblings named Wendy, John and Michael.",
      'Prince Otto, a Romance.jpg':
          "The novel is set in a fictional German principality called the Grand Duchy of Gerolstein. Prince Otto struggles hard to secure his throne and stabilize his country. But when Otto's throne is threatened, he must fight against various political intrigues and dangers to protect himself and his country.",
      'romola.jpg':
          'Romola, our main character, is an intelligent and beautiful woman. A mature and intelligent soul, Romola faces the betrayal of her husband Tito Melema and struggles to rebuild her life. ',
      'rose in bloom.jpg':
          'The novel is about the life of Rose Campbell, a young and talented woman. Rose is the character we met in the novel "Eight Cousins" when she was fourteen. Now she is twenty-one and the focus is on her relationships with the people around her, her love life and her personal development.',
      'the light.jpg':
          'It tells the story of war through the eyes of Jean Diaz, a French soldier. As Diaz fights alongside other soldiers on the front line, he confronts the terrible realities of war. The senselessness of war, the fear of death, and the worst in human beings are among the things Diaz and the other soldiers experience.',
      'under fire.jpg':
          'The novel tells the story of a French battalion during a year at the front, the daily life of the soldiers and their confrontation with the terrible realities of war.',
      'wanders.jpg':
          "The novel tells the story of Ove's adventures and the various characters he encounters while traveling through the Norwegian countryside. Ove is someone who does not conform to the norms of society and opposes traditional ways of life. ",
      'mor salkimli ev.jpg':
          " The author begins the book by describing the events she remembers moment by moment as a child. At first, her memories include the Purple Wisteria House and her mother. Her memories of her mother do not last long because she lost her mother at a young age. Perhaps the most important memory she remembers about her mother's death is the saffron-colored cloth she saw at her mother's funeral.",
      'ruh adam.jpg':
          'The novel tells the story of a man who devoted his life to military service, his mental state after being discharged from the military profession, his fall into a love predicament, his struggle with his own ego due to being torn between his mind and his heart.',
      'she and allan.jpg':
          "The novel focuses on the character Ayesha, introduced in the previous novel 'She'. The story sheds light on Ayesha's past and her mysterious life.",
      'tatarcik.jpg':
          'The story centers on the life of Lale, a young and beautiful Turkish girl. Lale tries to find her own identity in the Ottoman society, which is under the influence of Westernization and modernization movements, while at the same time trying to find a balance between traditional values and modern life.',
      'peter pan in kensington gardens.jpg':
          "The story begins with Peter Pan's disappearance as a baby and continues with his fantastic adventures in Kensington Gardens. Peter meets the other children in the gardens, talks to the animals and is drawn into a magical world.",
      'sherlock holmes.jpg':
          'Holmes is a clever and careful detective, using his observations and logic to solve cases. In the many stories of the series, Holmes and Watson confront murders, robberies, missing persons and mysterious events on the streets of London and other places of the period. ',
      'the idiot.jpg':
          'It tells the story of Prince Lev Nikolayevich Myshkin. Myshkin returns to Russia after a long period of treatment in Switzerland for a mental illness. However, he encounters difficulties in his social environment and among the people around him.',
      'Around the World in 80 Days.jpg':
          'Phileas Fogg makes a bet to travel around the world in 80 days, starting from London. Together with his servant Passepartout, they visit various countries and continents using trains, ships and other means of transportation. But along their journey, they encounter a detective and various challenges.',
      'robinson crouse.jpg':
          'The novel tells the story of Robinson Crusoe, an English sailor who is shipwrecked on a remote island and tries to survive there alone. Crusoe spends years on the island, struggling to survive, building shelters, finding food and filling his time with various activities to avoid feeling alone on the island.',
      'Gullivers travel.jpg':
          ' Gulliver goes on four different journeys: Lilliput, Brobdingnag, Laputa and Houyhnhnms. Each country has a different social structure, culture and worldview. Through his experiences in these different societies, Gulliver discovers human nature, social structures and human weaknesses.',
      'aydakı kadın.jpg':
          'Leyla is a childless married woman. First she has sex with Suat, then she marries Refik with an inexplicable state of mind. ',
      'emma.jpg':
          'Emma is a novel about youthful pride and romantic misunderstandings. The novel is set in the fictional village of Highbury and the surrounding estates of Hartfield, Randalls and Donwell Abbey and is about the relationships between several families.',
      'kurk mantolu madonna.jpg':
          'Raif Efendi is an introverted, melancholic character who has difficulty adapting to the outside world. He has endured many hardships throughout his life and failed to stand up against injustice. He married a woman he did not love and had children, but he could not live a life according to his own wishes. He wrote in his diary the only happy moment he ever really felt.',
      'Love and Mr Lewisham.jpg':
          "At the beginning of the novel, Mr. Lewisham is an 18-year-old teacher at a boys' school in Sussex, earning forty pounds a year. He meets and falls in love with Ethel Henderson, who is visiting relatives.",
      'madame bovary.jpg':
          'The novel deals with the events and relationships experienced by Emma Bovary, the wife of Charles Bovary, a fair-hearted and conventional physician with high ideals and a love of excessive pomp, in order to escape from monotony.The author adopts the observant side of realism while describing the inner worlds of the figures.',
      'simdi sevisme vakti.jpg':
          "It deeply affects the reader by touching the lives, emotions and relationships of people wandering the streets of Istanbul. With their sad, melancholic and sometimes ironic tones, Sait Faik's short stories skillfully portray the inner world of human beings and the complexity of life.",
      'a modest proposal.jpg':
          "It is an essay by the Irish writer Jonathan Swift, published in 1729. It was written as a critical response to the British Empire's rule of Ireland. While addressing the poverty of Ireland and the negligence of the British government, Swift also draws attention to social problems in a humorous style.",
      'a vital problem of married.jpg':
          'Gibbs explores the challenges of the eighth year of marriage and how couples can navigate this sensitive period.',
      'Alices Adventures in Wonderland.jpg':
          "Alice was bored sitting by the river with her sister. And there was nothing to do. Once or twice she glanced at the book in her sister's hand, but she could not see any pictures or dialog, 'But what good is a book without pictures or dialog?' she thought.",
      'An Elegy on the Glory of her Sex.jpg':
          'Goldsmith was an 18th century English poet and this poem focuses on the place and importance of women in society. The poem is an elegy written on the death of a woman named Mary Blaize.',
      'A Connecticut Yankee in King Arthurs Court.jpg':
          ' The novel tells the story of a Connecticut man named Hank Morgan. One day, he accidentally falls into the time of King Arthur, in medieval England. Hank Morgan decides to transform medieval society by utilizing modern knowledge and technology.',
      'A Short History of the World.jpg':
          ' A Short History of the World is a work on the history of humanity by the British writer H. G. Wells.',
      'a tale of two cities.jpg':
          'A Tale of Two Cities is a novel written by Charles Dickens in 1859 to be published in newspapers and set in Paris and London during and before the French Revolution. ',
      'A Tangled Tale.jpg':
          'A collection of 10 humorous short stories by Lewis Carroll, published serially in The Monthly Packet between April 1880 and March 1885.',
      'treatise of human nature.jpg':
          'A Treatise on Human Nature: Being an Attempt to Introduce the Method of Experimental Reasoning into Moral Matters is a book by the Scottish philosopher David Hume.',
      'a vısıt to three fonts.jpg':
          'During the war, Doyle visited the fronts of France, Belgium and Italy to see the true face of the war. It is a series written after these visits and reveals the brutality, human drama and courage of the war.',
      'adventures in journalism.jpg':
          "It is a book written by Philip Gibbs. Philip Gibbs is a British journalist and author. It is a memoir of Gibbs' experiences and observations during his journalism career.",
      'Adventures of Huckleberry Finn.jpg':
          " It focuses on Huck Finn's time spent with Jim, a runaway slave, and his inner struggle against slavery. Huck experiences a conflict between the values imposed on him by society and his own conscience, and finally decides to fight for Jim's freedom. ",
      'alemdagda bir yilan.jpg':
          "Published in 1950, this book is full of the author's short stories and usually deals with subjects such as nature, human relations, various segments of society and the streets of Istanbul.",
      'atesten gomlek.jpg':
          'The novel depicts the life of the main character Selma and the effects of the Turkish War of Independence. Selma tries to find her own identity and beliefs in the midst of war and social turmoil.',
      'bleak house.jpg':
          'On the one hand, the novel revolves around the case of Jarndyce and Jarndyce, a wealthy inheritance case, and on the other hand, it explores the lives and relationships of the different characters involved in this case.',
      'Boom, peace and quiet.jpg': '',
      'candide.jpg':
          'Candide is a young man known for his ignorant optimism and defends the thesis of a philosopher named Pangloss that "this world is the best world". However, Candide is forced to face the realities of life and loses his optimism as a result of the tragedies he experiences.  ',
      'an enquiry concerning human understanding.jpg':
          'Hume investigates how human beings acquire knowledge through information and experience and examines the limits of the human mind. In the work, he deals with topics such as causality, identity, space-time, causality and probability.',
      'an enquiry concerning the principles of morals.jpg':
          "While investigating the foundations of morality, Hume examines human nature and moral values. According to him, moral values are rooted not only in reason or God's will, but also in human nature.",
      'Ann Veronica.jpg':
          'Ann Veronica is a young woman full of the desire for independence. Challenging social norms and gender roles, she decides to lead her own life and make her own decisions.',
      'balkan harbi.jpg':
          ' It is about one of the wars the Ottoman Empire fought in the Balkans. While Ömer Seyfettin usually describes the effects of war on people and the tragedies it creates, this story shows the destructive nature of war and how people change during war.',
      'barnaby rudge.jpg':
          "Barnaby is a simple and innocent young man, but his mother is full of dark secrets that haunt him. The novel explores Barnaby's inner world as well as the social and political upheavals he experiences.",
      'Benjamin Franklin.jpg':
          'In his book, Franklin presents excerpts from his own life and teaches readers valuable lessons on the road to success.',
      'bes sehir.jpg':
          "Ahmet Hamdi Tanpınar's essay in which he describes the cities of Ankara, Erzurum, Konya, Bursa, Istanbul, which were the coincidences of his life.",
      'cakicinin ilk kursunu.jpg':
          'The story of a famous bandit who lived in Aydın, Çakırcalı Mehmet Efe or Çakıcı Efe',
      'daniel deronda.jpg':
          'The novel explores different segments of British society, Jewish identity, and the impact of the British Empire, while exploring the personal and social development of the characters.',
      'dirilen iskelet.jpg':
          'It reveals various sections of Ottoman society and the social structures of the period. While the novel depicts the lives, relationships and conflicts of different characters living in Istanbul, it also reflects the difficulties faced by the lower classes of society.',
      'dream psychology.jpg':
          'Freud argues that dreams are the expression of unconscious desires and that dreams use a symbolic language. According to him, deciphering the meaning of dreams is a way of revealing unconscious desires and anxieties.',
      'bozkurtlar diriliyor.jpg':
          'While describing the founding leader of the Göktürk State, Bilge Kagan and the events of his period, he emphasizes the national identity and spirit of the Turkish nation.',
      'bozkurtlarin olumu.jpg':
          'The novel is about the Battle of Malazgirt and its aftermath, a critical period in Turkish history. While describing the victory of the Turks over the Byzantine Empire in this battle, the novel also emphasizes the national identity and spirit of the Turkish nation.',
      'bugunun saraylisi.jpg':
          "'The Sarayi of Today' is about the lives and relationships of characters from various social classes living in different neighborhoods of Istanbul. The novel examines the characters' daily struggles, loves, hopes and disappointments, while also reflecting the political and social atmosphere of the period.",
      'butun oykuler.jpg':
          "love, freedom, justice, loneliness and despair... Sabahattin Ali's series of stories with simple and impressive narration, characters and events that deeply affect the reader.",
      'capitale de la douleur.jpg':
          'is a collection of poems by the French poet Paul Éluard, published in 1926. In this collection, Éluard expresses his personal pain, trauma and emotional experiences in the post-World War I period.',
      'classic for chilldren.jpg':
          'While the novel is about the life and relationships of a modern American family, it also alludes to classical works and ancient Greco-Roman culture.',
      'cleopatra.jpg':
          'Cleopatra mixes historical action with supernatural events and can be described as a historical fantasy novel.',
      'Confidence.jpg':
          "Francie is torn between two men living in Europe with different social positions: Bernard Longueville, a rich and attractive young man, and Lord Lovelace, an English nobleman. Francie's relationship between these two men and her emotional turmoil are at the center of the novel.",
      'david copperfield.jpg':
          'After being bullied by his mother Clara and stepfather Mr. Murdstone, David goes to live with his mother and her maid Peggotty. Facing many twists and turns in his life, David works in different professions, builds various relationships and tries to cope with the difficulties of life.',
      'Death in Venice.jpg':
          "Gustav von Aschenbach is a successful German writer who comes to Venice for a summer vacation. However, his obsessive attraction to a beautiful young Polish boy causes a deep inner breakdown. As she struggles with her passionate desires and inner conflicts, she must also contend with Venice's corrupt and death-stricken atmosphere.",
      'Der Zeuberberg.jpg':
          'Hans Castorp goes to a sanatorium in the Swiss Alps with a visiting relative and plans to spend a few weeks there. However, after his short stay, he unexpectedly stays in the sanatorium for several years. During this time, he develops relationships with various patients and doctors and becomes more and more immersed in this isolated world.',
      'dialogues concerning natural religion.jpg':
          'In the work, two characters, Cleanthes and Philo, defend natural theology, while the third character, Demea, takes a more mystical and mysterious approach to religious beliefs. The debate between these three characters deals with fundamental issues in the philosophy of religion, such as the provability of divine existence, the nature of the concept of God, and the existence of evil and suffering in the world.',
      'divine comedy.jpg':
          "It describes Dante's own spiritual journey and reflects the poet's profound reflections on the nature of Divine Justice and the spiritual salvation of man. Dante's work consists of three parts: Inferno (Inferno), Purgatory (Purgatorio) and Paradise (Paradiso). In each section, Dante encounters prominent historical and mythological figures, and through these encounters he describes the human journey between heaven and hell.",
      'dombey and son.jpg':
          "Paul Dombey is the son of Mr. Dombey, a rich and ruthless businessman. Mr. Dombey sees his son as a tool to strengthen his presence in the business world. However, Paul's poor health and failure to develop according to his expectations triggers a series of events in the life of the Dombey family.",
      'don quixote.jpg':
          "It tells the story of a Spanish nobleman named Alonso Quixano who gets carried away by romantic novels of chivalry and embarks on chivalric adventures, calling himself 'Don Quixote'. Together with his sidekick Sancho Panza, Don Quixote fights imaginary enemies, defends goodness and justice, and encounters the funny and twisted sides of the real world.",
      'Dorothy amd the Wizard Oz.jpg':
          "It tells the story of Dorothy Gale's return from Kansas to Oz, where she has new adventures. This time Dorothy, her Kansas pet Toto and her friend Zeb are trapped in an underground cave that was swallowed by an earthquake. They then encounter various fantastical creatures and events as they escape.",
      'Edinburgh.jpg':
          "It is a work by Scottish writer Robert Louis Stevenson published in 1878. It is a travel book containing Stevenson's observations and impressions of the city of Edinburgh.",
      'great expectations.jpg':
          "Pip is a poor peasant boy. One day, he is mysteriously invited to the mansion of Miss Havisham, a mysteriously wealthy person. There he meets Estella, Miss Havisham's beautiful but cold-hearted adopted daughter. Pip's life changes unexpectedly after this event and he becomes a man with 'great expectations'.",
      'eight cousins.jpg':
          "After the death of her father, Rose moves in with her aunt and uncle, where she grows up with her eight cousins. The novel is about the relationships between Rose's family and her cousins, her adventures and her growing up.",
      'fair margaret.jpg':
          "Margaret falls in love with Hugh Hainault, heir to a noble family. However, Hugh's mother looks down on Margaret's family and rejects them. This strains the relationship between Margaret and Hugh and leads to a series of tragic events.",
      'faust 1.jpg':
          "Faust tells the story of a scholar's pact with the devil named Mephistopheles and the events that follow. Faust is introduced as a scholar in search of more knowledge and meaning. Mephistopheles promises Faust eternal knowledge and pleasure, but asks for his soul in return. Faust accepts this offer and experiences a series of events on human experience.",
      'faust 2.jpg':
          "Faust tells the story of a scholar's pact with the devil named Mephistopheles and the events that follow. Faust is introduced as a scholar in search of more knowledge and meaning. Mephistopheles promises Faust eternal knowledge and pleasure, but asks for his soul in return. Faust accepts this offer and experiences a series of events on human experience.",
      'henry v.jpg':
          "After taking the throne of England, Henry declares war on France and wins a great victory at the Battle of Agincourt. However, Henry's leadership and decisions had a profound impact on both his soldiers and his enemies.",
      'huzur.jpg':
          'While Mümtaz questions himself and the values he belongs to in the complex atmosphere of Istanbul, he is also forced to confront his past and his relationship with love.',
      'First and Last Things.jpg':
          'Wells seeks the balance between scientific facts and human inner experiences and criticizes the complexity of the modern world.',
      'Fundamental Principles of the Metaphysic of Morals.jpg':
          'Kant finds the basis of moral obligations in universal moral principles, which he calls the "categorical imperative". According to him, our moral actions derive from our ability to accept moral laws as universal, and the essence of moral values is the adoption of our moral imperatives by our free will.',
      'general-introduction-to-psychoanalysis.jpg':
          'Freud deals with topics such as the unconscious, the interpretation of dreams, defense mechanisms and the basic concepts of psychoanalysis.',
      'God the Invisible King.jpg':
          "Wells discusses the evolution and role of religion and the concept of God. He also presents his ideas on how science and religion can complement each other and adapt to the modern world's understanding of religion.",
      'hamlet.jpg':
          "Hamlet is in a strange mood in the palace after the death of his father. He encounters his father's ghost and asks him to avenge his death. Hamlet experiences moral, philosophical and emotional conflicts while trying to fulfill his obligation to take revenge. ",
      'hard times.jpg':
          "Through the lives and interactions of various characters living in an industrial city called Coketown, we see the social class divisions of the period, the difficulties of the working class, the criticism of the education system and its effects on people's empathy and human emotions.",
      'Hawthorne.jpg': '',
      'homage to catalonia.jpg':
          'In Homage to Catalonia, Orwell examines the complex political processes, conflicts and ideological divisions during the Spanish Civil War. He also criticizes the internal struggles on the Republican side and the influence of Soviet Union-backed communist groups.',
      'hospital sketches.jpg':
          'Alcott served as a nurse in the Union Army during the American Civil War. Based on these experiences, "Hospital Sketches" examines the medical conditions on the battlefield, the experiences of wounded soldiers and the effects of war on people. ',
      'Human, All Too Human.jpg':
          'Nietzsche examines human nature, social structures, moral values and cultural norms, but also the inner conflicts and contradictions of human beings. Nietzsche argues that man must openly recognize his weaknesses and flaws, and that accepting them can have a liberating effect.',
      'hunger.jpg':
          "The protagonist, living in poverty, finds himself in a dilemma between writing and working to survive. While focusing on themes such as poverty, hunger and social ostracization, the novel also explores the conflicts and dilemmas in the protagonist's inner world.",
      'In the Days of the Comet.jpg':
          'After a comet appears in the sky one day, Leadford begins to observe the social transformations and changing behavior of people around the world. The gas from the star destroys the evil in people and creates harmony and peace in the world.',
      'jos boys.jpg':
          'It follows the life of Jo March, one of the daughters of the March family, as an adult and the lives of the people around her. Jo establishes a school called Plumfield, where she takes care of the children while trying to raise them at the same time. ',
      'kinglear.jpg':
          "It begins with Lear's wish to divide his kingdom among his three daughters Goneril, Regan and Cordelia. However, after the responses from his daughters, Lear excludes his most loyal daughter Cordelia and shares his kingdom with his other two daughters. This decision triggers family conflicts and turns Lear's life upside down.",
      'markopasa ve yazilari.jpg':
          "The story 'Marko Pasha' is one of Sabahattin Ali's stories that criticizes Turkish society and makes us think deeply about human psychology.",
      'ibni batuta.jpg':
          "It tells the story of Ibn Battuta's more than 30 years of travel, his experiences in the vast geography stretching between east and west, trade routes, local governments and social structures.",
      'icmizde ki seytan.jpg':
          "The Devil Inside Us is a novel about the divorce process of two people whose views on life are completely different. The novel tells the story of Macide and Ömer's love and divorce. Ömer is a young man who is bored with life and everything.",
      'iffet.jpg':
          "Münevver Hanım is a chaste woman who adheres to her moral values. However, at some point in her life, she meets Mr. Halit, who is older than her, and an impossible love begins between them. This love passes through a great test as society and moral values take a stand against them.",
      'iliad.jpg':
          ' The events that begin when Paris kidnaps Helen, the wife of Menelaus, brother of the Greek king Agamemnon, lead to a protracted war. The Iliad tells the story of the period leading up to the end of the war through the rage of Achilleus, the leader of the Greek warriors, the heroism of Hector and the tragic fates of many other characters.',
      'in the penal colony.jpg':
          'an officer punishes his criminals by strapping them to a torture device. However, the functioning of this device and the method of punishment are highly unusual and open to criticism.',
      'israel potter.jpg':
          "While fighting in various stages of the American Revolution, Israel Potter was captured during the war and taken to England by British ships. The novel details Israel Potter's experiences in England and his years in exile.",
      'istanbulun ic yuzu.jpg':
          'While Halide Edib deals with the traditional structure, social norms and the changing and transforming world of Istanbul, she also reflects the political and social atmosphere of the turbulent last periods of the Ottoman Empire.',
      'jack and jill.jpg':
          'Jack and Jill are two close friends living in a small town. As the novel progresses through the various events, games and learning experiences they go through together, it also deals with the growth and maturity of the characters.',
      'julius-caesar.jpg':
          "The assassination by a group of Roman senators, including Caesar's former ally Brutus, causes a major shake-up in the political atmosphere of Rome. In the aftermath of this assassination, conflicts between the Roman people and the generals begin and shake Rome.",
      'kalp agrisi.jpg':
          "Feride falls in love with Cemil, a married man, and has to deal with the internal conflicts caused by this forbidden love. The novel depicts Feride's emotional ebbs and flows, her internal reckoning and her conflict with social norms.",
      'kasagi.jpg':
          'It deals with various aspects of Ottoman-era Turkish society and human relations. "Kaşağı," is an impressive story that deals with various aspects of village life, traditions and social structure.',
      'Kidnapped.jpg':
          "It tells the story of David's abduction by a relative after the death of his parents and the adventures that follow. David encounters pirates, befriends fugitives and has to overcome many difficulties. ",
      'Life on the Mississippi.jpg':
          'In this work, Twain analyzes in detail the importance of the Mississippi River, the surrounding society and the way of life by sharing his own experiences and observations. The work also emphasizes the economic, social and cultural importance of the Mississippi River.',
      'macbeth.jpg':
          "It tells the story of Macbeth's ambitious plans to seize the throne as a result of the prophecies of the three witches, his own ambition and the provocations of his wife Lady Macbeth. However, as a result of the crimes they commit on the way to this ambition, the couple experience inner conflicts and move towards collapse.",
      'MAHUR BESTE.jpg':
          'Suat Bey is a character who is a composer of Turkish music and his relationship with music and his inner world play an important role throughout the novel.',
      'mansfield park.jpg':
          "Fanny grows up in Mansfield Park, the estate of the Bertrams, a wealthy family. The novel is about Fanny's experiences after she is adopted by her family for the estate and the social dynamics within the family.",
      'metres.jpg':
          ' The novel is set in one of the poorest districts of Istanbul and focuses on the relationship of one of the main characters, Nuri Bey, with his mistress, with whom he has an illicit affair. The novel deals with the inner conflicts of human beings and social pressures by focusing on this relationship that conflicts with the social norms and moral values of the period.',
      'Mr. Britling Sees It Through.jpg':
          "After the outbreak of the war, Britling confronts the tragedies and social problems of war. The novel deals with Britling's experiences, inner conflicts and thoughts from the beginning to the end of the war, while also examining the effects of the war on his family and other characters around him.",
      'New Arabian.jpg':
          'It includes unusual and exciting stories set in different places and times. These stories deal with different themes such as adventure, romance, mystery and humor.',
      'Notes of a Son and Brother.jpg':
          "It contains James's memories and reflections on his family and childhood. The book is a journey of inner exploration into the author's family, childhood and family ties.",
      'martin chuzzlewit.jpg':
          "Martin is a wealthy and charming heir, yet arrogant and selfish. The novel depicts the turning points in Martin's life, his relationships with his family, his loves and conflicts, as well as various cross-sections of English society at the time.",
      'medari maiset motoru.jpg':
          'The novel revolves around a group of characters from different neighborhoods of Istanbul. These characters come from different social classes and livelihoods and represent the complex fabric of Istanbul.',
      'memleket hikayeleri.jpg':
          'It presents cross-sections from different regions and people of Anatolia. The stories usually deal with themes such as human nature, moral problems, social conflicts and national values, based on the daily lives of ordinary people and the events they experience.',
      'merchant-of-venice.jpg':
          "The play is set in Venice and tells the story of one of the main characters, the merchant Antonio, who is unable to pay his debt to the Jewish moneylender Shylock. The play is also about Antonio's close friend Bassanio's love affair with Portia, a rich woman, and his struggle to win her inheritance.",
      'micromeas.jpg':
          'is about two alien beings, Micromégas and his eight-kilometer-tall friend, who visit Earth. These aliens come from different planets and when they meet humans, they begin to observe their small minds and prejudices.',
      'midsummernight.jpg':
          'While exploring the fine line between love, imagination and reality, it also examines the interaction of different social classes and characters. The play is notable for its complex plot, rich characters and poetic language, as well as its playful and magical atmosphere.',
      'moby dick.jpg':
          'The novel centers on Ahab, the captain of the whaling ship Pequod, and his obsession with hunting Moby Dick, a legendary white whale. Ahab is a sailor whose leg was once amputated by Moby Dick and sets out on a hunting trip with his ship to seek revenge.',
      'moon of israel.jpg':
          'While the novel deals with the ancient history of Egypt and the Pharaonic period, it also deals with the life of the Jewish people in captivity and the exodus of the Israelites from Egypt. ',
      'mrs  dalloway in bond street.jpg':
          "Clarissa's inner monologues in her mind describe in detail the people around her and the atmosphere of the city. While shopping and watching the people around her, Clarissa is immersed in her past memories and emotions. ",
      'murebbiye.jpg':
          "The story follows the events through the eyes of Ayşe, the young daughter of the family. Ayşe's parents are a progressive family who have adopted Western education methods and want to raise their children in a modern way. However, Fatma Hanım, the governess (nanny) of the family, resists modernization by maintaining her commitment to traditional values and Islam.",
      'nada the lily.jpg':
          "The novel is set in ancient Egypt and tells of events during the reign of Pharaoh Meneptah. The story follows the life of Nada, a girl from the Zulu people. Nada acts with strength and determination to avenge her father's death and secure the freedom of her people.",
      'natural selection.jpg':
          "Natural Selection is a fundamental concept in Charles Darwin's theory of evolution. In this process, organisms with characteristics adapted to the environment have a better chance of survival and reproduction. In this way, genetic changes occur in populations over time and species evolve.",
      'new life.jpg':
          " It consists of poems in which Dante deals with the theme of love and death. Throughout the book, Dante describes his love for a woman named Beatrice and his emotional experiences under her influence. Beatrice is an idealized figure in Dante's poems and represents his spiritual salvation.",
      'nicholas nickleby.jpg':
          'Nicholas lives a difficult life with his mother and sister Kate. After the death of his father, the family is forced to struggle with financial difficulties. Nicholas takes responsibility for his mother and sister and goes to London to look for work. Here he has various adventures in a world full of educational institutions and fraud.',
      'Ninety Three.jpg':
          'The novel follows three different main story lines. The first is the rebellion and escape of Lantenac of La Vendeé, a ship captain. The second is the internal reckoning and difficult decisions of Cimourdain, the Republican general sent to put down the uprising. The third is the tragic story of a family, a father and son and the difficulties they face in the midst of uprising and civil war.',
      'notes from the underground.jpg':
          'The novel takes its name from the experiences of a rebellious character, the narrator, in the underworld. The narrator is a character who goes against the norms and expectations of society. Feeling himself outside of society and rejecting its values, this character experiences a great conflict in his own inner world.',
      'our with spain for cuba freedom.jpg':
          "Trumbull White provides a detailed account of the events leading up to the war and examines the conflicts, political and social dynamics during the war. He explores the motivations behind America's entry into the war and analyzes what was being done on a large scale in Cuba's quest for freedom.",
      'Protagoras.jpg':
          "Protogoras is one of the representatives of the sophists and is known as the master of rhetoric and argumentation. Socrates has an in-depth discussion with Protogoras on ethics and knowledge. Throughout the dialogue, Socrates questions concepts such as virtue, justice, goodness and wisdom and criticizes the sophists' arguments.",
      'sehrın dısındakıler.jpg':
          'Hayri has moved away from the cultural and intellectual environment he lived in Istanbul and retreats to a quiet and peaceful life outside the city. However, he encounters various people and events in the town and tries to find himself in this new environment.',
      'Notre Dame de Paris.jpg':
          "The main characters of the novel include a hunchbacked man named Quasimodo, the beautiful and naive Esmeralda, Princess Phoebus and Bishop Claude Frollo. The story tells of Quasimodo's love for Esmeralda and Frollo's obsessive pursuit of her. ",
      'now it can be told.jpg':
          "It is a book of British writer Philip Gibbs' memoirs about the First World War, published in 1920. It is based on Gibbs' experiences as a journalist during the war.",
      'oliver twist.jpg':
          'The story follows Oliver from his birth to his placement in an orphanage and his subsequent encounters with malicious orphanage officials and criminal gangs living on the streets. Despite his hardships on the streets, Oliver is a kind and innocent character. ',
      'our mutual friend.jpg':
          'The novel begins with the death of a wealthy heir named John Harmon and the subsequent intrigues between the people who want to claim his inheritance. According to his will, Harmon is expected to leave this inheritance to a family that has done him a favor. However, Harmon escaped from the hands of those who tried to kill him and started to live under another identity. ',
      'oyle bir hikaye.jpg':
          "Sait Faik's stories are usually set in the streets of Istanbul and deal with the lives, emotions and conflicts of ordinary people. He often deals with the effects of nature and the sea on people and usually carries a melancholic atmosphere. ",
      'poems of william blake.jpg':
          'A collection of poems by the English poet, painter and printmaker William Blake. William Blake is one of the leading poets of the Romantic period and his work is known for its contemplation, mysticism and experimental use of language.',
      'pride and prejudice.jpg':
          'It centers on the search for marriage among the five daughters of the Bennet family. The story follows Elizabeth Bennet, one of the main characters, as she deals with her romantic relationships, her search for love and the expectations of society. Elizabeth meets the proud and wealthy Mr. Darcy and develops a bumpy relationship with him.',
      'resurrection.jpg':
          'It tells the story of Dmitri Nekhlyudov, an aristocratic youth, and his remorse after he accidentally causes the unjust imprisonment of a woman, and his attempt to redeem and atone for her. Nekhlyudov witnesses the dramatic transformation of this woman, Maslova, while at the same time strengthening his bond with her and undergoing a life-changing inner transformation.',
      'Rhyme and Reason.jpg':
          "The book contains poems written in a variety of themes and styles. Carroll's poems are full of language games, logic games and satirical references, while using a playful style. ",
      'Roderick Hudson.jpg':
          "The novel centers on the life and career of American sculptor Roderick Hudson. Hudson leaves America for Italy, where he is recognized as a talented artist. However, Hudson's career and personal relationships are fraught with personal difficulties as well as artistic ambitions and passions.",
      'romeo-and-juliet.jpg':
          'Romeo and Juliet are the children of two hostile families, the Montagues and the Capulets. However, these two young lovers feel a deep passion for each other despite the enmity of their families. Despite their passionate love, through a tragic series of events they find themselves involved in a suicide plot.',
      'saatlerı ayarlama enstıtusu.jpg':
          "The novel tells the story of a day in Istanbul and follows the lives and relationships of several different characters. The story is linked to Sabahattin Ali's famous story 'The Madonna with the Fur Coat' and one of the main characters, Hayri Irdal, makes several references to Sabahattin Ali's works.",
      'salommbo.jpg':
          "The story tells of a time when Carthage was struggling with internal and external dangers that threatened its existence. A rebellion breaks out in Carthage and Salammbô, the daughter of Carthage's general Hamilcar Barca, develops an emotional attachment to the rebel leader Matho. ",
      'saray ve otesi.jpg':
          'The story is told through the eyes of female characters from different walks of life in and around the Ottoman palace. The women living in the palace come from a variety of classes and statuses and each carries her own personal story and experience.',
      'St. Ives.jpg':
          'The novel is about the adventures of St. Ives, who becomes involved in a French espionage network during the Napoleonic Wars. St. Ives embarks on a journey full of dangers to escape captivity and find love.',
      'Sylvie and Bruno.jpg':
          'The novel revolves around two fantastic characters. Sylvie is described as a fairy girl, while Bruno is a fictional character who interacts with humans. The story has a complex structure that intertwines the fantastic adventures of Sylvie and Bruno with real world events.',
      'the american.jpg':
          "At the center of the novel is an American billionaire named Christopher Newman. While traveling in Europe, Newman meets and falls in love with Claire de Cintré, the daughter of a French aristocratic family. However, the interests and traditional expectations of Claire's family stand in the way of Newman and Claire's relationship.",
      'semaver.jpg':
          "Sait Faik's masterful narration and deep observation skills offer readers the opportunity to discover the deep emotions and human relationships hidden in the ordinary.",
      'sense and sensibility.jpg':
          'Focusing on the lives of Elinor and Marianne, the two daughters of the Dashwood family, the novel explores the balance between emotional and rational responses, focusing on the different characters of the two girls. Elinor is a rational and calm character, while Marianne is emotional and passionate in nature. The novel revolves around the love lives and relationships of these two girls.',
      'sevda sokagi komedyasi.jpg':
          'The novel is set in Sevda Street, where characters from different social classes and cultural backgrounds in Istanbul live. The story focuses on neighborhood relations, family conflicts and love.',
      'seyehatname.jpg':
          ' It is considered one of the oldest and most comprehensive travelogues in world literature. It provides a detailed and colorful portrait of the natural beauty, architecture, people and cultural details of the places Evliya Çelebi visited. ',
      'silas marner.jpg':
          "It deals with themes such as the inner transformation of man, social exclusion, moral values and human relationships. The novel is famous for George Eliot's in-depth character analysis and social criticism. ",
      'sinekkli bakkal.jpg':
          'The novel revolves around an inn called "Sinekli Bakkal" located in one of the various districts of Istanbul in the late 19th century, during the last periods of the Ottoman Empire. It centers on the life and story of Ihsan Bey, the owner of the inn, who observes his relations with various tenants. ',
      'sipsevdi.jpg':
          'The main character of the novel is Meftun Bey. Meftun Bey stays in Paris, France for years, where he goes to study, but he forgets the reason he came here and runs away from his education.',
      'songs of innocence.jpg':
          'The collection of poems revolves around the theme of innocence and childhood. Blake emphasizes the purity and innocence of childhood, leading to deep reflections on human nature.',
      'tempest.jpg':
          "Prospero has been exiled to an island where he uses his magical powers to control its inhabitants. The play revolves around Prospero's desire for revenge against his enemies, the love life of his daughter Miranda and the interactions of supernatural beings.",
      'The Adventures of Tom Sawyer.jpg':
          "The novel takes place in St. Petersburg, a small town on the banks of the Mississippi River. The main character, Tom Sawyer, is a local boy. Tom's adventures enliven the quiet life of the town and offer a portrait of his imaginative world and that of his friends.",
      'The Ambassadors.jpg':
          'international relations, cultural conflicts, personal freedoms and emotional attachments. The novel explores the differences and contradictions between America and Europe, while also exploring the inner conflicts and emotional development of the individual.',
      'The Antichrist.jpg':
          "It contains Nietzsche's thoughts on concepts such as nihilism, the will to power and transcendental man. The work presents an in-depth critique of how moral values and religion hinder human freedom and power.",
      'the beautiful and damned.jpg':
          'Anthony lives a rich life thanks to his inheritance, but this leads him to laziness and a bohemian life. Gloria, on the other hand, is in search of beauty and wealth. However, their marriage becomes monotonous and full of disappointments.',
      'The Black Arrow. A Tale Of Two Roses.jpg':
          "It is about the rivalry between the York and Lancaster dynasties in King Henry III's struggle for power. The story revolves around a young man, Dick Shelton. Dick learns of a political conspiracy that killed his family and then decides to find a black archer to take revenge.",
      'the bostanians.jpg':
          "Olive is a woman with feminist and reformist views and one of the pioneers of the women's rights movement. The main event of the novel is Olive's attempt to influence Verena Tarrant, a female activist, and to mold her in line with her ideals. ",
      'The Cossacks.jpg':
          'The novel follows the story of Olenin, a young Russian nobleman. Olenin is tired of the oppressive rules and limitations of his society and decides to seek freedom and adventure among the Cossacks in the Caucasus. ',
      'The Exemplary Novels of Cervantes.jpg':
          'It is a work by Spanish writer Miguel de Cervantes, published in 1613. It is a novella collection of twelve short stories and is considered an important contribution of Cervantes to Spanish literature.',
      'The Door in the Wall.jpg':
          'As a child, Lionel often discovers a secret door in a wall and finds a wonderful garden and a world full of journeys. However, each time his chance to visit this fantastical world is short-lived, and over time the real world begins to overpower him.',
      'the great gatsby.jpg':
          "Gatsby is a man living in opulence and has gained not only wealth but also social status in the hope of reconnecting with his forbidden love, Daisy Buchanan. However, Gatsby's life and love are dramatically confronted with the emptiness and corruption of the American Dream",
      'the brothers karamazoc.jpg':
          'The novel revolves around the three sons of the Karamazov family, Dmitri, Ivan and Alyosha. It tells a complex story centered around themes such as family conflicts, moral complexities, religion and existential questions, love and crime. ',
      'The Burning Secret.jpg':
          'The story centers on the complex relationship between a mother and son. It revolves around Edgar, a young boy whom Baron, a handsome and charming man, meets in a hotel during a summer vacation and tries to establish a secret relationship.',
      'The Chartreuse of Parma.jpg':
          "Fabrice is a character living during the Napoleonic Wars and is confronted with the political and social events of the French Revolution and the rise of Napoleon. Fabrice's passionate love and adventurous life form the main plot of the novel.",
      'the confidence.jpg':
          'Set on a cruise ship, the novel depicts the interactions and discussions of various characters. among the characters is the Trust Man, a con man who constantly changes his identity. it shows how the con man acts in various disguises and how he affects the characters, while offering a critical look at various aspects of American society.',
      'the crime and punishment.jpg':
          'Raskolnikov is a law student living in poverty and has complex thoughts about the upper class of society. These thoughts lead him to plot the murder of a wealthy loan shark. ',
      'The Crime of the Congo.jpg':
          'It is a book by Irish journalist and writer Sir Arthur Conan Doyle, published in 1909. It was written to draw attention to the crimes of slavery and colonialism in the Congo under the rule of King Leopold II of Belgium.',
      'The Critique of Pure Reason.jpg':
          'The book presents a critical examination that explores the limits of human knowledge and the process of acquiring knowledge. Kant explores the possibilities of rational thought and reason, while discussing the limits of experience and the nature of knowledge that can be attained through reason.',
      'the desert village.jpg':
          'The poem describes the abandonment of an Irish village and the destruction of rural life. Goldsmith offers a profound critique of the effects of modernization and the decline of rural life. ',
      'The First Men In The Moon.jpg':
          "The novel follows the two main characters, Bedford and Cavor, on a fantastic journey to the Moon together. Cavor is a genius scientist and has invented a spaceship to explore the Moon. Bedford joins Cavor's project and together they embark on an adventurous journey to the Moon.",
      'The Food of the Gods.jpg':
          'The novel revolves around two main characters, Mr. Bensington and Mr. Redwood. The two discover a growth hormone and decide to test its effects on animals. However, these experiments unexpectedly spiral out of control and the discovered nutrient creates animals that grow much faster than normal and reach gigantic sizes.',
      'The Fur Country.jpg':
          'The story follows a group of intrepid adventurers as they attempt to establish a fur trading station in the cold and harsh conditions of Hudson Bay. While having to deal with polar expeditions, encounters with indigenous people and the challenges of the wilderness, they also tackle issues such as the conservation of natural resources and the sustainability of the trade.',
      'the gambler.jpg':
          "Alexei risks his love and even his material possessions because of his passion for gambling. Throughout the novel, Alexei's casino adventures and emotional turmoil are described, while at the same time providing an in-depth analysis of social class differences and human nature.",
      'the genealogy of morals.jpg':
          "It is a work by the German philosopher Friedrich Nietzsche, published in 1887. This book presents Nietzsche's in-depth analysis of the origin and evolution of moral values.",
      'the gold bug.jpg':
          'Legrand is a man living in Virginia who one day finds a treasure map by simply following the trail of an insect. This map points to a hidden treasure once used by pirates.',
      'the golden bowl.jpg':
          "The novel revolves around the marriage of American art collector Adam Verver and his daughter Maggie Verver to the British nobleman Prince Amerigo. However, the story also includes Prince Amerigo's passion for another woman to whom he is married, Charlotte Stant, and the complex relationships that this passion unravels. ",
      'The great boer war.jpg':
          'Discussing the conflicts between the British Empire and the Boer Republics, military strategies and the various battlefields, this book also discusses the social, political and military consequences of the war.',
      'The History Of Mr. Polly.jpg':
          'After a series of events, Alfred quits his job and starts looking for different adventures. However, his escape attempts have unexpected consequences and Alfred is forced to re-evaluate his own life. In the end, he embarks on an inner journey to find his own happiness.',
      'The Hunting of the Snark.jpg':
          'The book tells the story of a group of travelers on an adventure to hunt the "Snark", a mysterious and imaginary creature. The elite group, under the command of their leader Bellman, pursues the Snark and encounters various fantastic and bizarre adventures.',
      'The Innocents Abroad, travel.jpg':
          "Mark Twain offers an entertaining and sometimes cynical view of the societies and cultures of Europe and the Middle East at the time. Twain's sense of humor and satirical flair makes readers both laugh and think, and is an important example of the travel writing genre.",
      'the history of england i.jpg':
          "Hume's work of history deals with the various political events of the period, the monarchy, parliament, religious conflicts and the general development of England. However, Hume's understanding of history is not as detailed and document-oriented as that of modern historians. ",
      'the history of england ii.jpg':
          "Hume's work of history deals with the various political events of the period, the monarchy, parliament, religious conflicts and the general development of England. However, Hume's understanding of history is not as detailed and document-oriented as that of modern historians. ",
      'the history of england iii.jpg':
          "Hume's work of history deals with the various political events of the period, the monarchy, parliament, religious conflicts and the general development of England. However, Hume's understanding of history is not as detailed and document-oriented as that of modern historians. ",
      'the history of england iv.jpg':
          "Hume's work of history deals with the various political events of the period, the monarchy, parliament, religious conflicts and the general development of England. However, Hume's understanding of history is not as detailed and document-oriented as that of modern historians. ",
      'the history of england v.jpg':
          "Hume's work of history deals with the various political events of the period, the monarchy, parliament, religious conflicts and the general development of England. However, Hume's understanding of history is not as detailed and document-oriented as that of modern historians. ",
      'the history of england vi.jpg':
          "Hume's work of history deals with the various political events of the period, the monarchy, parliament, religious conflicts and the general development of England. However, Hume's understanding of history is not as detailed and document-oriented as that of modern historians. ",
      'the inferno.jpg':
          'is about the experiences of a group of soldiers who embark on a journey into the hell of war. As the soldiers face the terrible realities of war, they also discover the darker aspects of human nature. ',
      'The Invisible Man.jpg':
          "The novel begins when Griffin, a scientist, develops a formula that will make him invisible. However, after this experiment, the ability to become invisible completely changes Griffin's life and makes him uncontrollable.",
      'The Island of Dr. Moreau.jpg':
          "Edward the sailor is shipwrecked and washed up on Dr. Moreau's island, where a scientist is conducting horrific experiments on animals by manipulating human genes. Moreau's half-human, half-animal creatures create a tense atmosphere on the island.",
      'the journal to stella.jpg':
          "His relationship with Esther Johnson constitutes an important part of Swift's life. The letters contain Swift's personal thoughts, feelings and comments on the political events of the period. Through the letters, Swift often expresses to Stella how he thinks and feels about her.",
      'the king solomon.jpg':
          'Together with a group of adventurers, Quatermain embarks on a dangerous journey deep into Africa. On this adventurous journey, the heroes face various dangers and search for legendary riches.',
      'The Man in the Brown Suits.jpg':
          'Anne is bored with her ordinary life in London and decides to travel to Africa to add excitement to her life. The death of a mysterious man she meets on a cruise ship leads Anne into a series of dangerous events.',
      'the masque of the red death.jpg':
          'The story is set in a country plagued by a deadly epidemic called the red death, where Prince Prospero, a wealthy nobleman, decides to live an isolated life in his castle to protect himself from the epidemic. He organizes a masquerade ball in the castle and his guests have fun isolated from the outside world to protect themselves from the epidemic. However, one night at the ball, a figure appears: "The Red Death" himself.',
      'The Murder on the links.jpg':
          "The story begins with the murder of Renauld, a rich French man. Hercule Poirot arrives on the scene and sets out to solve the murder. With Renauld's death, various suspicions and mysteries arise. ",
      'The Mysterious Island.jpg':
          'Set during the American Civil War, a balloon used by a group of fugitives to escape the war is washed away by a hurricane and crashes on an island. The passengers of the balloon try to survive on this mysterious island in the South Pacific and discover its secrets.',
      'the portrait of dorian gray.jpg':
          'The main character, Dorian Gray, is a handsome young man. Basil Hallward, a painter, paints a portrait of Dorian and is mesmerized by his beauty and youth. Dorian is influenced by a friend, Lord Henry Wotton, and adopts a hedonistic lifestyle.',
      'The Prince and the Pauper.jpg':
          'Prince Edward is the son of King Henry VIII of England and lives a life of luxury. Tom Canty is from a poor family and lives near the palace. One day, they both discover that they have similar physical characteristics and decide to impersonate each other. However, this has unexpected consequences and both of their lives change drastically.',
      'The Sea Lady.jpg':
          'The main character of the novel, Nettie, a sea creature known as a mermaid, decides to come out of the sea and join humans to get to know them better. However, existing and being accepted in human society will not be as easy as she expects.',
      'The Time Machine.jpg':
          'It begins with a character, a scientist and inventor, who discovers the ability to travel through time with an invention he calls the Time Machine. Using the Time Machine, he embarks on a journey into the future. In the future, he discovers that different human species have evolved on Earth.',
      'the mystery marie roget.jpg':
          ' It begins with the mysterious disappearance of Marie Rogêt and the subsequent discovery of her dead body. The police, the press and the public show great interest in solving the murder, but the mystery remains unsolved. C. Auguste Dupin then decides to investigate the complex clues and evidence.',
      'the mystery of edwin drood.jpg':
          "Drood's disappearance worries readers and other characters, and a number of suspects emerge, including Rosa Bud, Drood's fiancée. As the novel nears its end, the mysterious events are expected to be solved, but the story is incomplete due to Dickens' sudden death.",
      'the mystery of the blue train.jpg':
          "The story begins with a murder on a blue train. The body of one of the train passengers, Ruth Kettering, attracts Poirot's attention and he tries to solve the murder. It turns out that a large inheritance is behind the murder and events unfold with a series of complex clues and suspects.",
      'The New Machiavelli.jpg':
          "Remington is determined to do whatever it takes to achieve political power and success. However, his ambition and passion lead him to face personal and moral challenges. As Remington tries to rise in the political arena by following Machiavelli's principles, he must also struggle with his own inner conflicts.",
      'the prince.jpg':
          'In the book, Machiavelli advises rulers to use all means to maintain and expand their power. He argues that idealistic and moral principles are insufficient in the real world and suggests that politics requires a pragmatic approach.',
      'the raven.jpg':
          'The story begins one winter night when a visitor arrives in the room where a man sits in solitude under the influence of his grief and depression. The visitor is "Raven", a crow. The crow questions the human soul and reflects the inner world of the man.',
      'The Red and The Black.jpg':
          'Julien comes from a low social class as the son of a priest, but is distinguished by his intelligence and desire to shape his own life. The novel begins with Julien taking a teaching position in the mansion of a wealthy family and then embarks on a journey full of aspirations for advancement and love affairs.',
      'The Research Magnificent.jpg':
          "Porphyry is known for his curiosity for knowledge and has deep thoughts about the future of humanity. The novel follows the turning points in Porphyry's life and his inner questioning, while examining how science and technology affect and change humanity.",
      'The Secret Adversary.jpg':
          'In 1919 London, demobilized soldier Tommy Beresford meets war volunteer Prudence "Tuppence" Cowley. Both unemployed and penniless. "They set up Young Adventurers, Ltd. Mr. Whittington follows Tuppence to present her work.',
      'The sleeper awakes.jpg':
          'Graham falls into a long sleep due to an illness and wakes up about two hundred years later. When he wakes up, the world has changed drastically; people live in big cities and society is run by a giant corporation.',
      'the soul of a nation.jpg':
          ' In his book, Gibbs examines the mood and social atmosphere of England during the First World War. Topics such as the effects of the war, the emotional reactions of the people and the deep effects of the war on society are discussed.',
      'The Thomas Jefferson writes.jpg':
          " These works reveal Jefferson's views, political ideas and ideals regarding specific events in American history. Jefferson's personal thoughts and feelings can also be found in these written works.",
      'the trial.jpg':
          'The story begins with the unexpected arrest and subsequent trial of the protagonist Josef K. However, K. does not know exactly what he is accused of and finds himself caught up in a complex legal system as he tries to understand why the process is happening.',
      'The Twilight of the Idols.jpg':
          'The book expands and deepens many of the themes Nietzsche deals with in his other works. Nietzsche criticizes moral values, religion and cultural norms. The book questions the foundations of Western civilization and discusses the collapse of human values.',
      'the voyage of the beagle.jpg':
          "The book describes Darwin's research, observations and discoveries in different geographical regions around the world. Darwin studied many different species and observed the relationships between different species in regions such as South America, the Galapagos Islands, Australia and New Zealand.",
      'the voyage out.jpg':
          'Rachel Vinrace joins a sea voyage from England to South America. During the voyage, Rachel meets different people and experiences a variety of experiences. Throughout the voyage, Rachel struggles to find her own identity and place, while reflecting on the relationships and social norms of the people around her.',
      'The War in the Air.jpg':
          'A British officer stumbles upon a revolutionary war plane invented by a German scientist he meets in a weather balloon. The plane is stolen for use and causes a worldwide war to break out.',
      'The war in the east japan.jpg':
          "Evans and Peattie examine in detail the various war strategies, operations and military campaigns of the Imperial Japanese Army. The book describes the Japanese army's attacks and occupation of China, Korea, Manchuria and other East Asian countries.",
      'the war of the worlds.jpg':
          'The story begins with a sudden Martian attack on Earth. The Martians destroy people and cities using their famous three-legged machines and deadly heat rails. The main character of the novel, an unnamed Englishman, narrates the events from where he lives.',
      'The Wheels of Chance.jpg':
          'The story is about Hoopdriver, a bicycle mechanic who goes on an unauthorized bicycle adventure. While living an ordinary life, Hoopdriver embarks on a romantic journey with a woman he meets by chance and falls in love with. ',
      'the whıte company.jpg':
          'The story follows the adventures of Alleyne Edricson, a brave young English soldier. Alleyne joins The White Company, a group of warriors dedicated to justice and chivalry. This group sets out to join the British army fighting the French.',
      'The wizard of wall street and his wealth.jpg':
          "This book tells the story of the life and fortune of John D. Rockefeller, a man of great influence in the financial world of the United States at the turn of the 20th century. The book examines Rockefeller's life from birth, his rise in business and his leadership in the oil industry. ",
      'The Wonderful Visit.jpg':
          'Set in an ordinary town in England, the story begins with an unexpected visit by an angel, Ariel. As Ariel wanders the world as an angel, she causes a series of interesting events in the town and leaves a deep impression on people.',
      'The World Set Free.jpg':
          'Set in the early 20th century, humanity discovers nuclear energy and uses it as a weapon. These weapons have devastating powers that destroy cities and change the world. ',
      'the wuthering heights.jpg':
          "The story begins with a mysterious orphan boy named Heathcliff being raised by the Earnshaw family. Heathcliff is introduced as a rough and tough boy, but over time his deep love for the family's daughter Catherine Earnshaw becomes the center of the story.",
      'the-sorrows-of-young-werther.jpg':
          'The novel is presented to the reader through the letters of the young and emotional Werther. As Werther expresses his love for the beautiful Charlotte, he also expresses his pain and conflicts. However, Charlotte has decided to marry another man.',
      'this side of paradise.jpg':
          "The novel depicts the life of Amory Blaine, a young and wealthy student at Princeton University. Focusing on Amory's education, relationships and personal growth, the story paints a portrait of American youth and society at the time.",
      'Three Sunsets and Other Poems.jpg':
          "Carroll's humorous and ironic style is also evident in these poems. The poem 'Three Sunsets' offers an in-depth reflection on nature and human emotions, while the other poems deal with different subjects.",
      'three-essays-on-the-theory-of-sexuality.jpg':
          'In the book, Freud provides a comprehensive analysis of the nature and development of sexuality and its effects on human psychology. Freud examines how sexual drives develop in childhood, the importance of sexual identity and sex life, and the social and cultural influences of sexuality.',
      'Tom Sawyer Abroad.jpg':
          'The story begins with Tom, Huck and Jim traveling around the world in an airship. However, the airship veers off course and they land on an island in a distant, unfamiliar land. Here, they encounter strange and funny events and interact with the natives of the island.',
      'Tom Sawyer, Detective.jpg':
          'Tom and Huck set out on an adventure to find a treasure that Jim, a former slave, has lost. Jim, who is the only person the duo can trace, had previously escaped on a charge of theft. Tom and Huck track Jim down to prove his innocence and have a series of adventures in the process.',
      'Tono-Bungay.jpg':
          "The novel is about the creation of a miraculous drug called 'Tono-Bungay', a kind of talisman and antidote, and the events surrounding it. It begins with the childhood memories of George Ponderevo, the nephew of this drug merchant, and then moves on to George's adulthood. ",
      'totem-and-taboo.jpg':
          'Totemism is a belief system in which a group of people feel attached to an animal or object and consider it sacred. Taboo is when society prohibits certain behaviors and considers them sacred.',
      'thus spake zarathustra.jpg':
          'Zarathustra retreats to the mountains where he spends ten years reflecting in order to bring a new system of morality and values to humanity. In the end, he brings to people the idea of the "Übermensch" (beyond-human) and proposes to go beyond traditional moral values.',
      'the white nights.jpg':
          "The novel is the story of four white nights spent by a young man in St. Petersburg with a young woman he meets and falls in love with. The young man meets Nastenka, a mysterious and melancholic woman, and confesses his feelings to her. However, there is another man in Nastenka's life.",
      'turkun atesle imtihani.jpg':
          'The book deals with the various stages of the national struggle and the heroism that took place during this period. Among the main characters is a group of young people from various social and economic backgrounds.',
      'Twenty Thousand Leagues Under the Sea.jpg':
          'The novel tells the adventures of Professor Aronnax, the marine biologist Conseil and the whaler Ned Land, who travel on a submarine called the Nautilus. The captain of the Nautilus is Captain Nemo, a mysterious merman. ',
      'Under Sentence of Death.jpg':
          'The story follows the endless adventures of a submarine crew after they are caught in a ship sinking operation. The crew is sentenced to death and imprisoned inside the submarine due to the fact that their ship sinking activities violate international law.',
      'Vicar of Wakefield.jpg':
          'Reverend Primrose is a meek and virtuous character, living a quiet life with her family. But through a series of unfortunate events, his family begins to fall on hard times.',
      'vurun kahpeye.jpg':
          'The novel tells the story of Aliye, an idealist teacher from Istanbul, who travels to a town in Anatolia and supports the idea of the National Struggle in the region. ',
      'What is Enlightenment.jpg': '',
      'white bird.jpg':
          'James has fallen deeply in love with a woman named Elspeth. However, Elspeth is engaged to David Webster, a man already at sea. Despite this, James continues to hope that Elspeth will eventually choose him.',
      'Wilhelm Meister.jpg':
          'Wilhelm Meister is a young man in search of self-discovery and self-realization. Although he comes from a working family, he is interested in the world of art and theater. ',
      'yasadigim gibi.jpg':
          "The book covers the period from Tanpınar's childhood to his adulthood. The author candidly informs the reader about his family, his education, his literary and intellectual interests, his relations with his environment and his experiences in the literary world.",
      'yeni dunya.jpg':
          'Consisting of 13 stories, the book is important as it shows that the author has reached a significant level of competence in the field of storytelling. In terms of content, apart from the town and village life, the stories dealing with fallen women have an important place.',
      'yezidin kizi.jpg':
          'A mysterious romance that begins during a cruise and continues in the vast Syrian desert...',
      'zadig.jpg':
          'Zadig encounters disasters at various periods of his life and these disasters shape his character. Throughout the story, Zadig searches for concepts such as wisdom, justice and truth and encounters different people in the process.',
      'kirik hayatlar.jpg':
          'The events in the book take place in the last period of the Ottoman Empire. In the last period of the Ottoman Empire, the Turkish people develop a blind wannabe for the West. Instead of the technology, science and science that should be taken from the West; the culture that is contrary to our lives is imitated.',
      'Italian Hours.jpg':
          'The events in the book take place in the last period of the Ottoman Empire. In the last period of the Ottoman Empire, the Turkish people develop a blind wannabe for the West. Instead of the technology, science and science that should be taken from the West; the culture that is contrary to our lives is imitated.',
      'little dorrit.jpg':
          'Amy Dorrit, the main character of the novel, is a young girl born and raised in Marshalsea Prison in London. Her father is a man who lives in prison because of his debts. ',
      'little men.jpg':
          "The story revolves around the school called Plumfield where Jo March, one of the members of the March family introduced in 'Little Women', lives. Jo and her husband Professor Bhaer open a children's school called Plumfield, where children of different ages study together. ",
      'little woman.jpg':
          'Jo March, one of the main characters, and her three sisters come from an ordinary family living during the American Civil War. The novel is about the difficulties, joys, loves and disappointments the sisters experience as they transition from adolescence to adulthood. ',
      'king-Lear.jpg':
          "It begins with Lear's wish to divide his kingdom among his three daughters Goneril, Regan and Cordelia. However, after the responses from his daughters, Lear excludes his most loyal daughter Cordelia and shares his kingdom with his other two daughters. This decision triggers family conflicts and turns Lear's life upside down.",
      // Add other image descriptions here
    },
    'tr': {
      'a modest proposal.jpg':
          "  İrlandalı yazar Jonathan Swift tarafından 1729 yılında yayımlanmış bir makaledir. Bu makale, İngiliz İmparatorluğu'nun İrlanda'yı yönetimine eleştirel bir yanıt olarak yazılmıştır. Swift, İrlanda'nın yoksulluğunu ve İngiliz hükümetinin ihmalkarlığını ele alırken, aynı zamanda mizahi bir üslupla toplumsal sorunlara dikkat çekmektedir.",
      'a vital problem of married.jpg':
          "Gibbs, evliliğin sekizinci yılının zorluklarını ve çiftlerin bu hassas dönemde nasıl yol alabileceğini araştırıyor. Bu eser akademisyenler tarafından kültürel açıdan önemli olduğu gerekçesiyle seçilmiştir ve bildiğimiz şekliyle medeniyetin bilgi tabanının bir parçasıdır.",
      'Alices Adventures in Wonderland.jpg':
          "Irmağın kıyısında ablasıyla oturmaktan Alice'in canı sıkılmıştı. Üstelik yapacak bir şey yoktu. Bir iki kere ablasının elindeki kitaba şöyle bir göz attıysa da ne resim görebilmişti ne de diyalog, 'İyi ama resimsiz ve diyalogsuz bir kitap neye yarar ki?' diye düşündü.",
      'An Elegy on the Glory of her Sex.jpg':
          "Goldsmith, 18. yüzyıl İngiliz şairlerinden biridir ve bu şiir, toplumda kadınların yerine ve önemine odaklanır.Şiir, Mary Blaize adlı bir kadının ölümü üzerine yazılmış bir ağıttır.",
      'beyond good and evil.jpg':
          "İyinin ve Kötünün Ötesinde: Gelecekteki bir Felsefeye Giriş, Alman filozof Friedrich Nietzsche tarafından yazılmış, 1886 yılında yayınlanmış kitaptır.",
      '2 Poems.jpg':
          "Oscar Wilde, 19. yüzyıl İrlandalı yazar, oyun yazarı ve şairdir. En çok tiyatro oyunları ve hicivli eserleriyle tanınır, ancak aynı zamanda şair olarak da önemli eserler bırakmıştır. Wilde'ın şiirleri, genellikle estetik ve duygusal derinlikle dikkat çeker ve genellikle ironi ve zeka ile işlenir.",
      '11 Intentions.jpg':
          "Oscar Wilde 19. yüzyıl İrlandalı yazar, oyun yazarı ve şairdir. En çok tiyatro oyunları ve hicivli eserleriyle tanınır, ancak aynı zamanda şair olarak da önemli eserler bırakmıştır. Wilde'ın şiirleri, genellikle estetik ve duygusal derinlikle dikkat çeker ve genellikle ironi ve zeka ile işlenir.",
      'A Connecticut Yankee in King Arthurs Court.jpg':
          " Roman, Hank Morgan adında bir Connecticut'lıyı anlatır. Bir gün, bir kaza sonucu Kral Arthur'un zamanına,  yani Orta Çağ İngiltere'sine düşer. Hank Morgan, modern bilgi ve teknolojiden yararlanarak, Orta Çağ toplumunu dönüştürmeye karar verir.",
      'A Short History of the World.jpg':
          " A Short History of the World, İngiliz yazar H. G. Wells'in insanlık tarihi üzerine yazdığı bir eserdir.",
      'a tale of two cities.jpg':
          "İki Şehrin Hikâyesi, Charles Dickens'ın 1859 yılında gazetelerde tefrika edilmek üzere yazdığı, konusu Fransız Devrimi esnasında ve öncesinde Paris ve Londra'da geçen romandır. ",
      'A Tangled Tale.jpg':
          "Lewis Carroll'un Nisan 1880 ile Mart 1885 arasında The Monthly Packet dergisinde seri olarak yayınlanan 10 kısa mizahi öyküsünden oluşan bir koleksiyon.",
      'a treatise of human nature.jpg':
          "İnsan Doğası Üzerine Bir İnceleme: Ahlaki Konulara Deneysel Akıl Yürütme Yöntemini Tanıtma Girişimi Olmak, İskoç filozof David Hume'un bir kitabıdır.",
      'a vısıt to three fonts.jpg':
          "Doyle, savaş sırasında savaşın gerçek yüzünü görmek için Fransa, Belçika ve İtalya cephelerini ziyaret etti. bu ziyaretlerin ardından kaleme alınan bir seridir ve savaşın vahşetini, insanlık dramını ve cesaretini gözler önüne serer.",
      'adventures in journalism.jpg':
          " Philip Gibbs tarafından yazılmış bir kitaptır. Philip Gibbs, İngiliz bir gazeteci ve yazardır. Gibbs'in gazetecilik kariyeri boyunca yaşadığı deneyimleri ve gözlemlerini içeren bir anı kitabıdır.",
      'Adventures of Huckleberry Finn.jpg':
          " Huck Finn'in kaçak köle Jim ile birlikte geçirdiği zamanı ve onun köleliğe karşı içsel mücadelesini odaklanır. Huck, toplumun ona dayattığı değerlerle kendi vicdanı arasında bir çatışma yaşar ve sonunda Jim'in özgürlüğü için mücadele etmeye karar verir.",
      'alemdagda bir yilan.jpg':
          "1950 yılında yayımlanan bu kitap, yazarın kısa öyküleriyle doludur ve genellikle doğa, insan ilişkileri, toplumun çeşitli kesimleri ve İstanbul'un sokakları gibi konuları ele alır.",
      'atesten gomlek.jpg':
          "Roman, ana karakter olan Selma'nın yaşadığı hayatı ve Türk Kurtuluş Savaşı'nın etkilerini anlatır. Selma, savaşın ve toplumsal çalkantıların ortasında kendi kimliğini ve inançlarını bulmaya çalışır.",
      'bleak house.jpg':
          " Roman, bir yanda varlıklı bir miras davası olan Jarndyce ve Jarndyce davası etrafında dönerken, diğer yanda bu davaya karışmış olan farklı karakterlerin hayatlarını ve ilişkilerini inceler.",
      'candide.jpg':
          "Candide, yüzündeki cahil iyimserlikle tanınan ve Pangloss adlı bir filozofun 'bu dünya en iyi dünya' tezini savunan bir gençtir. Ancak Candide, hayatın gerçekleriyle yüzleşmek zorunda kalır ve yaşadığı trajediler sonucunda iyimserliğini yitirir.",
      'an enquiry concerning human understanding.jpg':
          "Hume, insanın bilgi ve deneyim yoluyla nasıl bilgi elde ettiğini araştırır ve insan zihninin sınırlarını inceler. Eserde, nedensellik, özdeşlik, uzay-zaman, nedensellik ve olasılık gibi konuları ele alır.",
      'an enquiry concerning the principles of morals.jpg':
          "Hume, ahlakın temellerini araştırırken insan doğasını ve ahlaki değerleri inceler. Ona göre, ahlaki değerlerin kökeni sadece akıl veya Tanrı'nın iradesi değil, aynı zamanda insanın doğasından kaynaklanır.",
      'Ann Veronica.jpg':
          "Ann Veronica, bağımsızlık arzusuyla dolu bir genç kadındır. Toplumsal normlara ve cinsiyet rollerine meydan okuyarak, kendi hayatını yönlendirmeye ve kendi kararlarını almaya karar verir.",
      'Around the World in 80 Days.jpg':
          "Phileas Fogg, Londra'dan başlayarak dünyayı 80 günde dolaşmak için bir iddiaya girer. Hizmetçisi Passepartout ile birlikte  tren, gemi ve diğer ulaşım araçlarını kullanarak çeşitli ülkeleri ve kıtaları ziyaret ederler. Ancak yolculukları boyunca, bir dedektif ve çeşitli zorluklarla karşılaşırlar.",
      'balkan harbi.jpg':
          " Osmanlı İmparatorluğu'nun Balkanlar'da yaşadığı savaşlardan birini konu alır. Ömer Seyfettin'in genellikle savaşın insanlar üzerindeki  etkilerini ve yarattığı trajedileri anlattığı bu hikaye, savaşın yıkıcı doğasını ve insanların savaş sırasında nasıl değiştiğini gösterir.",
      'barnaby rudge.jpg':
          "Barnaby, basit ve masum bir gençtir, ancak annesi onu etkisi altına alan karanlık sırlarla doludur. Roman, Barnaby'nin yaşadığı toplumsal ve siyasi çalkantılarla birlikte onun içsel dünyasını da inceler.",
      'Benjamin Franklin.jpg':
          "  Franklin, kitabında kendi hayatından kesitler sunar ve okuyuculara başarıya giden yolda değerli dersler verir.",
      'bes sehir.jpg':
          "Ahmet Hamdi Tanpınar'ın, hayatının tesadüfleri olan Ankara, Erzurum, Konya, Bursa, İstanbul şehirlerini anlattığı deneme türü eseri.",
      'cakicinin ilk kursunu.jpg':
          " Aydın'da yaşamış olan ünlü bir eşkıyanın, Çakırcalı Mehmet Efe veya Çakıcı Efe denen eşkıyanın Hikâyesi.",
      'daniel deronda.jpg':
          "Roman, İngiliz toplumunun farklı kesimlerini, Yahudi kimliğini ve İngiliz İmparatorluğu'nun etkilerini incelerken, karakterlerin kişisel ve toplumsal gelişimlerini ele alır.",
      'dirilen iskelet.jpg':
          "Osmanlı toplumunun çeşitli kesitlerini ve dönemin sosyal yapılarını gözler önüne serer. Roman, İstanbul'da yaşayan farklı karakterlerin hayatlarını, ilişkilerini ve çatışmalarını anlatırken, toplumun alt sınıflarının yaşadığı zorlukları da yansıtır.",
      'dream psychology.jpg':
          "Freud, rüyaların bilinçaltındaki arzuların ifadesi olduğunu ve rüyaların sembolik bir dil kullandığını savunur. Ona göre, rüyaların anlamını çözmek, bilinçaltındaki istekleri ve endişeleri açığa çıkarmanın bir yoludur.",
      'bozkurtlar diriliyor.jpg':
          "  Göktürk Devleti'nin kurucu lideri Bilge Kağan ve onun döneminde yaşanan olayları anlatırken, Türk milletinin milli kimliğini ve ruhunu vurgular.",
      'bozkurtlarin olumu.jpg':
          " Türk tarihindeki kritik bir dönem olan Malazgirt Meydan Muharebesi'ni ve sonrasını konu alır. Bu savaşta Türklerin Bizans İmparatorluğu'na karşı elde ettiği zafer anlatılırken, roman aynı zamanda Türk milletinin milli kimliğini ve ruhunu vurgular.",
      'bugunun saraylisi.jpg':
          "Bugünün Saraylısı, İstanbul'un farklı semtlerinde yaşayan çeşitli sosyal sınıflardan karakterlerin hayatlarını ve ilişkilerini konu alır. Roman, karakterlerin günlük yaşam mücadeleleri, aşkları, umutları ve hayal kırıklıklarını incelerken, aynı zamanda dönemin siyasi ve toplumsal atmosferini de yansıtır.",
      'butun oykuler.jpg':
          "aşk, özgürlük, adalet, yalnızlık ve umutsuzluk... Sabahattin Ali'nin anlatımı sade ve etkileyici oykuleri, okuyucuyu derinden etkileyen karakterleri ve olayları ıle dolu oyku serısı.",
      'capitale de la douleur.jpg':
          " Fransız şair Paul Éluard'un 1926 yılında yayımlanan şiir koleksiyonudur. Éluard, bu koleksiyonda I. Dünya Savaşı sonrası dönemde yaşadığı kişisel acıları, travmaları ve duygusal deneyimleri dile getirir.",
      'classic for chilldren.jpg':
          " Roman, modern bir Amerikan ailesinin yaşamını ve ilişkilerini konu alırken, aynı zamanda klasik eserlere ve antik Yunan-Roma kültürüne bir gönderme yapar.",
      'cleopatra.jpg':
          "Kleopatra, tarihi aksiyonu doğaüstü olaylarla karıştırır ve tarihi bir fantastik roman olarak tanımlanabilir.",
      'Confidence.jpg':
          " Francie, Avrupa'da yaşayan ve toplumsal konumları farklı olan iki adam arasında kalmıştır: zengin ve çekici bir genç adam olan Bernard Longueville ile İngiliz bir soylu olan Lord Lovelace. Francie'nin bu iki adam arasındaki ilişkisi ve duygusal karmaşaları romanın merkezinde yer alır.",
      'david copperfield.jpg':
          " Annesi Clara ve üvey babası Mr. Murdstone'un yanında zorbalık gören David, sonradan annesi ve annesinin hizmetçisi Peggotty ile yaşamaya başlar. Hayatında birçok dönemeçle karşılaşan David, farklı mesleklerde çalışır, çeşitli ilişkiler kurar ve hayatın zorluklarıyla başa çıkmaya çalışır.",
      'Death in Venice.jpg':
          " Gustav von Aschenbach, Venedik'e yaz tatili için gelen başarılı bir Alman yazarıdır. Ancak, burada karşılaştığı genç ve güzel bir Polonyalı erkek çocuğuna duyduğu saplantılı ilgi, iç dünyasında derin bir çöküşe neden olur. Tutkulu arzuları ve içsel çatışmalarıyla boğuşurken, aynı zamanda Venedik'in bozulmuş ve ölümle iç içe geçmiş atmosferiyle de mücadele etmek zorundadır.",
      'Der Zeuberberg.jpg':
          "Hans Castorp, ziyaret ettiği akrabasıyla birlikte İsviçre Alpleri'ndeki bir sanatoryuma giderek orada birkaç hafta geçirmeyi planlar. Ancak, kısa süreli ziyaretinin ardından beklenmedik bir şekilde birkaç yıl boyunca sanatoryumda kalır. Bu süre zarfında, çeşitli hastalarla ve doktorlarla ilişkiler kurar ve zamanla kendini bu izole dünyaya daha fazla kaptırır.",
      'dialogues concerning natural religion.jpg':
          "Eserde, iki karakter olan Cleanthes ve Philo, doğal teolojiyi savunurken, üçüncü karakter olan Demea ise dini inançları daha mistik ve gizemli bir şekilde ele alır. Bu üç karakter arasındaki tartışma, tanrısal varlığın kanıtlanabilirliği, tanrı kavramının doğası, dünya üzerindeki kötülük ve acıların varlığı gibi temel din felsefesi konularını ele alır.",
      'divine comedy.jpg':
          " Dante'nin kendi ruhsal yolculuğunu anlatır ve şairin İlahi Adalet'in (Divine Justice) doğası ve insanın ruhsal kurtuluşu üzerine derin düşüncelerini yansıtır. Dante'nin eseri, üç bölümden oluşur: Cehennem (Inferno), Araf (Purgatorio) ve Cennet (Paradiso). Her bölümde Dante, önde gelen tarihî ve mitolojik figürlerle karşılaşır ve bu karşılaşmalar aracılığıyla insanın cennet ve cehennem arasındaki yolculuğunu anlatır.",
      'dombey and son.jpg':
          "Paul Dombey, zengin ve acımasız bir iş adamı olan Mr. Dombey'in oğludur. Mr. Dombey, oğlunu iş dünyasındaki varlığını güçlendirmek için bir araç olarak görür. Ancak, Paul'un sağlığı zayıf olduğu için beklentilerine uygun bir şekilde gelişmemesi, Dombey ailesinin yaşamında bir dizi olayı tetikler.",
      'don quixote.jpg':
          "Alonso Quixano adlı bir İspanyol soylusunun, kafasını romantik şövalyelik romanlarına kaptırması ve kendisini Don Quixote olarak adlandırarak şövalyelik maceralarına atılmasını konu alır. Don Quixote, yandaşı Sancho Panza ile birlikte hayali düşmanlarla savaşır, iyiliği ve adaleti savunurken gerçek dünyanın komik ve çarpık yanlarıyla karşılaşır.",
      'Dorothy amd the Wizard Oz.jpg':
          "Dorothy Gale'in Kansas'tan Oz'a geri dönmesini ve orada yeni maceralar yaşamasını anlatır. Bu sefer Dorothy, Kansas'taki evcil hayvanı olan Toto ve ozamanlardaki arkadaşı Zeb ile birlikte bir deprem sırasında yuttukları yer altı mağarasında mahsur kalır. Daha sonra buradan kaçarken çeşitli fantastik yaratıklar ve olaylarla karşılaşırlar.",
      'Edinburgh.jpg':
          "  İskoç yazar Robert Louis Stevenson'ın 1878 yılında yayımladığı bir eserdir. Bu eser, Stevenson'ın Edinburgh şehrine dair gözlemlerini ve izlenimlerini içeren bir seyahat kitabıdır.",
      'great expectations.jpg':
          "Pip, fakir bir köylü çocuğudur. Bir gün, gizemli bir şekilde zengin bir kişi olan Bayan Havisham'ın malikanesine gizemli bir şekilde davet edilir. Orada, Bayan Havisham'ın güzel ancak soğuk kalpli evlatlık kızı Estella ile tanışır. Pip'in hayatı, bu olaydan sonra beklenmedik bir şekilde değişir ve o artık büyük beklentileri olan biri haline gelir.",
      'simdi sevisme vakti.jpg':
          "İstanbul'un sokaklarında dolaşan insanların hayatlarına, duygularına ve ilişkilerine dokunarak okuyucuyu derinden etkiler. Hüzünlü, melankolik ve bazen ironik tonlarıyla Sait Faik'in kısa öykülerinde insanın iç dünyası ve yaşamın karmaşıklığı ustalıkla işlenir.",
      'Gullivers travel.jpg':
          " Gulliver, dört farklı seyahatte bulunur: Lilliput, Brobdingnag, Laputa ve Houyhnhnms ülkelerine gider. Her ülke, farklı bir toplum yapısına, kültüre ve dünya görüşüne sahiptir. Gulliver, bu farklı toplumlarda yaşadığı deneyimler aracılığıyla insan doğasını, toplumsal yapıları ve insanlığın zayıflıklarını keşfeder.",
      'eight cousins.jpg':
          " Rose, babasının ölümünden sonra teyzesi ve amcasının yanına taşınır ve burada sekiz kuzeniyle birlikte büyür. Roman, Rose'un ailesi ve kuzenleri arasındaki ilişkileri, maceraları ve büyüme sürecini konu alır.",
      'fair margaret.jpg':
          "Margaret, soylu bir ailenin varisi olan Hugh Hainault'a aşık olur. Ancak, Hugh'un annesi, Margaret'ın ailesini aşağı görür ve onları reddeder. Bu durum, Margaret ve Hugh arasındaki ilişkiyi zorlar ve bir dizi trajik olaya neden olur.",
      'faust 1.jpg':
          " bir bilginin, Mephistopheles adlı şeytanla yaptığı anlaşmayı ve sonrasında yaşadığı olayları anlatır. Faust, daha fazla bilgi ve anlam arayışında olan bir bilgin olarak tanıtılır. Mephistopheles, Faust'a sonsuz bilgi ve zevk vaat eder ancak karşılığında ruhunu ister. Faust, bu teklifi kabul eder ve insan deneyimi üzerine bir dizi olay yaşar.",
      'faust 2.jpg':
          " bir bilginin, Mephistopheles adlı şeytanla yaptığı anlaşmayı ve sonrasında yaşadığı olayları anlatır. Faust, daha fazla bilgi ve anlam arayışında olan bir bilgin olarak tanıtılır. Mephistopheles, Faust'a sonsuz bilgi ve zevk vaat eder ancak karşılığında ruhunu ister. Faust, bu teklifi kabul eder ve insan deneyimi üzerine bir dizi olay yaşar.",
      'henry v.jpg':
          " Henry, İngiltere tahtına geçtikten sonra Fransa'ya savaş açar ve Agincourt Muharebesi'nde büyük bir zafer kazanır. Ancak, Henry'nin liderliği ve kararları, hem askerleri hem de düşmanları üzerinde derin etkiler bırakır.",
      'huzur.jpg':
          " Mümtaz, İstanbul'un karmaşık atmosferinde kendini ve ait olduğu değerleri sorgularken, aynı zamanda geçmişiyle ve aşkla olan ilişkisiyle yüzleşmek zorunda kalır.",
      'First and Last Things.jpg':
          " Wells, bilimsel gerçekler ve insanın içsel deneyimleri arasındaki dengeyi arar ve modern dünyanın karmaşıklığına yönelik eleştirilerde bulunur.",
      'Fundamental Principles of the Metaphysic of Morals.jpg':
          "Kant, ahlaki yükümlülüklerin temelini kategorik imperatif olarak adlandırdığı evrensel ahlaki prensiplerde bulur. Ona göre, ahlaki eylemlerimiz, ahlaki yasaları evrensel olarak kabul etme yeteneğimizden kaynaklanır ve ahlaki değerlerin özü, ahlaki zorunluluklarımızın özgür irademiz tarafından benimsenmesidir.",
      'general-introduction-to-psychoanalysis.jpg':
          "Freud, bilinçdışı, rüyaların yorumlanması, savunma mekanizmaları ve psikanalizin temel kavramları gibi konuları ele alır.",
      'God the Invisible King.jpg':
          " Wells dinin ve tanrı kavramının evrimini ve rolünü tartışır. Ayrıca, bilimin ve dinin birbirini nasıl tamamlayabileceği ve modern dünyanın din anlayışına nasıl uyum sağlayabileceği üzerine fikirlerini sunar.",
      'hamlet.jpg':
          " Hamlet, babasının ölümünün ardından sarayda tuhaf bir ruh hali içindedir. Babasının hayaletiyle karşılaşır ve ondan ölümünün intikamını almasını ister. Hamlet, intikam alma yükümlülüğünü yerine getirmeye çalışırken ahlaki, felsefi ve duygusal çatışmalar yaşar.",
      'hard times.jpg':
          "Coketown adında bir endüstri şehrinde yaşayan çeşitli karakterlerin hayatları ve etkileşimleri üzerinden, dönemin toplumsal sınıf ayrımlarını, işçi sınıfının zorluklarını, eğitim sisteminin eleştirilerini ve insanların empati ve insani duyguları üzerindeki etkilerini görmekteyiz.",
      'homage to catalonia.jpg':
          "Orwell, Homage to Catalonia'da, İspanya İç Savaşı sırasında yaşanan karmaşık politik süreçleri, çatışmaları ve ideolojik bölünmeleri inceler. Aynı zamanda, Cumhuriyetçi tarafındaki iç mücadeleleri ve Sovyetler Birliği destekli komünist grupların etkisini de eleştirir.",
      'hospital sketches.jpg':
          "Alcott, Amerikan İç Savaşı sırasında Union Army'de bir hemşire olarak görev yapmıştır. Hospital Sketches, bu deneyimlerine dayanarak savaş alanındaki tıbbi koşulları, yaralı askerlerin yaşadıklarını ve savaşın insanlar üzerindeki etkilerini inceler.",
      'Human, All Too Human.jpg':
          " insan doğasını, toplumsal yapıları, ahlaki değerleri ve kültürel normları ele alırken, aynı zamanda insanın içsel çatışmalarını ve çelişkilerini de inceler. Nietzsche,  insanın acizliklerini ve kusurlarını açıkça görmesi gerektiğini ve bunları kabul etmenin özgürleştirici bir etki yaratabileceğini savunur.",
      'hunger.jpg':
          " yoksulluk içinde yaşayan Baş karakter, hayatta kalmak için yazma ve çalışma arasında gidip gelen bir çıkmazda kendisini bulur. Yoksulluk, açlık ve toplum tarafından dışlanma gibi temalar üzerine odaklanırken, aynı zamanda roman, baş karakterin içsel dünyasındaki çatışmaları ve çıkmazları da inceler.",
      'In the Days of the Comet.jpg':
          "Leadford, bir gün gökyüzünde görülen bir kuyruklu yıldızın ardından dünya üzerindeki toplumsal dönüşümleri ve insanların değişen davranışlarını gözlemlemeye başlar. Yıldızın gazı, insanların içlerindeki kötülüğü yok eder ve dünyada bir uyum ve barış ortamı sağlar. ",
      'jos boys.jpg':
          "March ailesinin kızlarından biri olan Jo March'ın yetişkinliğindeki yaşamını ve çevresindeki insanların hayatlarını ele alır. Jo, Plumfield adlı bir okul kurar ve bu okulda çocuklarla ilgilenirken, aynı zamanda onları yetiştirmeye çalışır. ",
      'kinglear.jpg':
          "Lear'ın üç kızı Goneril, Regan ve Cordelia arasında krallığını paylaştırma isteğiyle başlar. Ancak, kızlarından gelen yanıtların ardından Lear, en sadık kızı Cordelia'yı dışlar ve krallığını diğer iki kızıyla paylaşır. Bu karar, aile içi çatışmaları tetikler ve Lear'ın hayatını altüst eder.",
      'markopasa ve yazilari.jpg':
          "Marko Paşa başlıklı öykü, Sabahattin Ali'nin Türk toplumunu eleştiren ve insan psikolojisi üzerine derinlemesine düşündüren hikayelerinden biridir.",
      'ibni batuta.jpg':
          " İbn Battuta'nın 30 yılı aşkın süren gezi macerasını, doğu ve batı arasında uzanan geniş coğrafyada yaşadığı deneyimleri, ticaret yollarını, yerel yönetimleri ve toplumsal yapıları aktarır.",
      'icmizde ki seytan.jpg':
          " Ahmet Celal, toplumun ve kendi iç dünyasının baskıları altında ezilmiş, tükenmiş bir adamdır. Roman, onun geçmişine ve yaşamının farklı dönemlerine geri dönerek, okuyucuya karakterin içsel çatışmalarını ve yabancılaşmasını anlatır.",
      'iffet.jpg':
          " Münevver Hanım, ahlaki değerlerine bağlı, iffetli bir kadındır. Ancak, hayatının bir döneminde kendisinden yaşça büyük olan Halit Bey ile karşılaşır ve aralarında imkansız bir aşk başlar. Bu aşk, toplumun ve ahlaki değerlerin onlara karşı tavır almasıyla büyük bir sınavdan geçer.",
      'iliad.jpg':
          " Paris'in, Yunan kralı Agamemnon'un kardeşi Menelaos'un eşi olan Helen'i kaçırmasıyla başlayan olaylar, uzun süren bir savaşa neden olur. İlyada, savaşın sona ermesine kadar olan dönemi, Yunan savaşçılarının önderi Achilleus'un öfkesi, Hektor'un kahramanlığı ve diğer birçok karakterin trajik kaderleri üzerinden anlatır.",
      'in the penal colony.jpg':
          "bir subay, yaptığı suçluları işkence cihazına bağlayarak onlara ceza verir. Ancak, bu cihazın işleyişi ve cezalandırma yöntemi oldukça sıradışıdır ve eleştirilmeye açıktır.",
      'israel potter.jpg':
          " İsrail Potter, Amerikan Devrimi'nin çeşitli aşamalarında savaşırken, savaş sırasında esir düşer ve İngiliz gemileri tarafından İngiltere'ye götürülür. Roman, İsrail Potter'ın İngiltere'de yaşadığı deneyimleri ve sürgündeki yıllarını detaylı bir şekilde aktarır.",
      'istanbulun ic yuzu.jpg':
          "Halide Edib, İstanbul'un geleneksel yapısını, toplumsal normlarını ve değişen dönüşen dünyasını ele alırken, aynı zamanda Osmanlı İmparatorluğu'nun çalkantılı son dönemlerindeki siyasi ve toplumsal atmosferi de yansıtır.",
      'jack and jill.jpg':
          "Jack ve Jill, küçük bir kasabada yaşayan iki yakın arkadaştır. Roman, onların birlikte yaşadıkları çeşitli olaylar, oyunlar ve öğrenme deneyimleri üzerinden ilerlerken, karakterlerin büyüme ve olgunlaşma süreçlerini de ele alır.",
      'julius-caesar.jpg':
          " Caesar'ın eski müttefiki Brutus'un da aralarında bulunduğu bir grup Roma senatörü tarafından gerçekleştirilen suikast, Roma'nın siyasi atmosferinde büyük bir sarsıntıya neden olur. Bu suikastın ardından, Roma halkı ve generaller arasında çatışmalar başlar ve Roma'yı sarsar.",
      'kalp agrisi.jpg':
          "Feride, evli bir adam olan Cemil'e aşık olur ve bu yasak aşkın yarattığı iç çatışmalarla başa çıkmak zorunda kalır. Roman, Feride'nin duygusal gelgitlerini, iç hesaplaşmalarını ve toplumsal normlarla çatışmasını anlatır.",
      'kasagi.jpg':
          "Osmanlı dönemi Türk toplumunun çeşitli yönlerini ve insan ilişkilerini ele alır. Kaşağı, köy yaşamı, gelenekler ve toplumsal yapının çeşitli yönlerini işleyen etkileyici bir hikayedir.",
      'Kidnapped.jpg':
          "David'in ailesinin ölümünden sonra, akrabası tarafından zorla gemiye bindirilerek kaçırılmasını ve ardından yaşadığı serüvenleri anlatır. David, korsanlarla karşılaşır, kaçaklarla dost olur ve birçok zorluğun üstesinden gelmek zorunda kalır. ",
      'Life on the Mississippi.jpg':
          "Twain, bu eserde kendi deneyimlerini ve gözlemlerini aktararak, Mississippi Nehri'nin önemini, çevresindeki toplumu ve yaşam tarzını detaylı bir şekilde inceler. Ayrıca, eserde Mississippi Nehri'nin ekonomik, sosyal ve kültürel önemine de vurgu yapılır.",
      'macbeth.jpg':
          "Macbeth'in, üç cadının kehanetleri sonucu kendi hırsı ve karısı Lady Macbeth'in kışkırtmalarıyla tahtı ele geçirmek için yaptığı hırslı planları anlatır. Ancak, bu hırs yolunda işledikleri suçların sonucunda, çift içsel çatışmalar yaşar ve çöküşe doğru ilerlerler.",
      'MAHUR BESTE.jpg':
          "Suat Bey, Türk müziği bestecisi olan bir karakterdir ve roman boyunca müzikle olan ilişkisi ve içsel dünyası önemli bir yer tutar.",
      'mansfield park.jpg':
          "Fanny, zengin bir aile olan Bertram'ların malikanesi Manşfield Park'ta büyümektedir. Roman, Fanny'nin ailesi tarafından bu malikane için evlatlık alınmasının ardından yaşadığı deneyimleri ve ailenin içindeki sosyal dinamikleri konu alır.",
      'metres.jpg':
          "Roman, İstanbul'un fakir semtlerinden birinde geçer ve baş karakterlerinden biri olan Nuri Bey'in, yasak bir ilişki yaşadığı metresiyle olan ilişkisini konu alır. Roman, dönemin toplumsal normları ve ahlaki değerleriyle çatışan bu ilişkiyi merkeze alarak, insanın içsel çatışmalarını ve toplumsal baskıları işler.",
      'Mr. Britling Sees It Through.jpg':
          " Britling, savaşın patlak vermesinin ardından, savaşın getirdiği trajedilerle ve toplumsal sorunlarla yüzleşir. Roman, Britling'in savaşın başlangıcından sonuna kadar yaşadığı deneyimleri, içsel çatışmalarını ve düşüncelerini ele alırken, aynı zamanda savaşın etkilerini ailesi ve çevresindeki diğer karakterler üzerinde de inceler.",
      'New Arabian.jpg':
          "New Arabian Nights, farklı mekânlarda ve zamanlarda geçen, sıradışı ve heyecan verici hikayeleri içerir. Bu hikayelerde, macera, romantizm, gizem ve mizah gibi farklı temalar işlenir.",
      'Notes of a Son and Brother.jpg':
          "James'in ailesi ve çocukluk dönemiyle ilgili anılarını ve düşüncelerini içerir. Kitap, yazarın ailesine, çocukluk yaşantısına ve aile bağlarına yönelik içsel bir keşif yolculuğunu anlatır.",
      'martin chuzzlewit.jpg':
          " Martin, zengin ve çekici bir varisken, kibirli ve bencil bir yapıya sahiptir. Roman, Martin'in hayatındaki dönüm noktalarını, ailesiyle olan ilişkilerini, aşklarını ve çekişmelerini anlatırken, aynı zamanda dönemin İngiliz toplumunun çeşitli kesitlerini de işler.",
      'medari maiset motoru.jpg':
          "Roman, İstanbul'un farklı semtlerinden gelen bir grup karakterin etrafında dönüyor. Bu karakterler, farklı sosyal sınıflardan ve geçim kaynaklarından gelmekte olup, İstanbul'un karmaşık dokusunu temsil ederler.",
      'memleket hikayeleri.jpg':
          "Anadolu'nun farklı bölgelerinden ve insanlarından kesitler sunar. Hikayeler, genellikle sıradan insanların günlük yaşamlarından ve yaşadıkları olaylardan yola çıkarak, insan doğası, ahlaki sorunlar, toplumsal çatışmalar ve milli değerler gibi temaları ele alır.",
      'merchant-of-venice.jpg':
          "Oyun, Venedik'te geçer ve ana karakterlerden biri olan tüccar Antonio'nun, Yahudi tefeci Shylock'a olan borcunu ödeyememesi üzerine gelişen olayları anlatır. Oyun, aynı zamanda Antonio'nun yakın arkadaşı Bassanio'nun, zengin bir kadın olan Portia ile olan aşkını ve onun mirasını kazanmak için verdiği mücadeleyi de konu alır.",
      'micromeas.jpg':
          "Micro-Megas, iki uzaylı varlığın, Micromégas ve sekiz kilometre boyundaki arkadaşı, dünyayı ziyaret etmelerini konu alır. Bu uzaylılar, farklı gezegenlerden gelirler ve insanlarla karşılaştıklarında onların küçük zekâlarını ve ön yargılarını gözlemlemeye başlarlar.",
      'midsummernight.jpg':
          " aşk, hayal gücü ve gerçeklik arasındaki ince çizgiyi keşfederken, aynı zamanda farklı toplumsal sınıfların ve karakterlerin etkileşimini de inceler. Oyun, karmaşık bir kurgusu, zengin karakterleri ve şiirsel dilinin yanı sıra eğlenceli ve büyülü atmosferiyle de dikkat çeker.",
      'moby dick.jpg':
          "Roman, bir balina avcısı gemisi olan Pequod'un kaptanı Ahab'ın, efsanevi bir beyaz balina olan Moby Dick'i avlama saplantısını konu alır. Ahab, daha önce Moby Dick tarafından bacağı koparılan bir denizcidir ve intikam almak için gemisiyle bir av gezisine çıkar.",
      'moon of israel.jpg':
          "Roman, Mısır'ın eski tarihini ve Firavun dönemini konu alırken, aynı zamanda Yahudi halkının esaret altındaki yaşamını ve İsrailoğulları'nın Mısır'dan çıkışını da ele alır. ",
      'mrs  dalloway in bond street.jpg':
          "Clarissa'nın zihnindeki iç monologları, çevresindeki insanları ve şehrin atmosferini detaylı bir şekilde anlatır. Clarissa, alışveriş yaparken ve çevresindeki insanları izlerken, geçmiş anılarına ve duygularına dalar. ",
      'murebbiye.jpg':
          "Mürebbiye, ailenin küçük kızı Ayşe'nin gözünden olayları takip eder. Ayşe'nin anne ve babası, Batılı eğitim metotlarını benimsemiş ve çocuklarını modern bir şekilde yetiştirmek isteyen ilerici bir ailedir. Ancak, ailenin mürebbiyesi (dadısı) Fatma Hanım, geleneksel değerlere ve İslamiyet'e bağlılığını koruyarak modernleşmeye direnir.",
      'nada the lily.jpg':
          "Roman, antik Mısır'da geçer ve Firavun Meneptah dönemindeki olayları anlatır. Hikaye, Zulu halkından gelen bir kız olan Nada'nın yaşamını konu alır. Nada, babasının intikamını almak ve kendi halkının özgürlüğünü sağlamak için güçlü ve kararlı bir şekilde hareket eder.",
      'natural selection.jpg':
          "Doğal Seçilim, Charles Darwin'in evrim teorisinde temel bir kavramdır. Bu süreçte, çevreye uyumlu özelliklere sahip organizmaların hayatta kalma ve üreme şansı daha yüksektir. Bu sayede, popülasyonlarda zamanla genetik değişiklikler meydana gelir ve türler evrim geçirir.",
      'new life.jpg':
          " Dante'nin aşk ve ölüm temasını işlediği şiirlerden oluşur. Dante, kitap boyunca Beatrice adlı kadına duyduğu aşkı ve onun etkisi altında yaşadığı duygusal deneyimleri anlatır. Beatrice, Dante'nin şiirlerinde idealize edilmiş bir figür olarak yer alır ve onun ruhsal kurtuluşunu temsil eder.",
      'nicholas nickleby.jpg':
          "Nicholas, annesi ve kardeşi Kate ile zorlu bir hayat sürmektedir. Babası öldükten sonra, aile ekonomik sıkıntılarla mücadele etmek zorunda kalır. Nicholas, annesi ve kardeşi için sorumluluk alır ve Londra'ya giderek iş aramaya başlar. Burada, eğitim kurumları ve sahtekarlıkla dolu dünyada çeşitli maceralar yaşar.",
      'Ninety Three.jpg':
          "Roman, üç farklı ana hikaye hattını takip eder. Birincisi, bir gemi kaptanı olan La Vendeé'li Lantenac'ın isyanı ve kaçışıdır. İkincisi, ayaklanmanın bastırılması için gönderilen Cumhuriyetçi general Cimourdain'in iç hesaplaşmaları ve zor kararlarıdır. Üçüncüsü ise bir ailenin trajik hikayesidir; bir baba ve oğlunun ayaklanma ve iç savaşın ortasında karşı karşıya kaldıkları zorlukları anlatır.",
      'notes from the underground.jpg':
          "Roman, adını anlatıcı olarak adlandırdığı isyankar karakterin yeraltı dünyasında yaşadığı deneyimlerden alır. Anlatıcı, toplumun normlarına ve beklentilerine karşı çıkan bir karakterdir. Kendini toplumun dışında hisseden ve toplumun değerlerini reddeden bu karakter, kendi iç dünyasında büyük bir çatışma yaşar.",
      'our with spain for cuba freedom.jpg':
          "Trumbull White savaşa yol açan olayların ayrıntılı bir anlatımını sunar ve savaş sırasında yaşanan çatışmaları, siyasi ve sosyal dinamikleri inceler. Amerika'nın savaşa girişinin arkasındaki motivasyonları araştırır ve Küba'nın özgürlük arayışı için geniş çapta nelerin yapıldığını analiz eder.",
      'Protagoras.jpg':
          "Protogoras, sofistlerin temsilcilerinden biridir ve retoriğin ve argümantasyonun ustası olarak bilinir. Sokrates, Protogoras ile etik ve bilgi üzerine derinlemesine bir tartışma yapar. Diyalog boyunca, Sokrates, erdem, adalet, iyilik ve bilgelik gibi kavramları sorgular ve sofistlerin argümanlarını eleştirir.",
      'romola.jpg':
          "Hikaye, genç ve idealist bir kadın olan Romola'nın hayatını merkeze alır. Romola, zengin bir tüccar olan babası Baldassarre'nin trajik bir şekilde kaybolmasının ardından, idealist prens Savonarola'nın etkisi altında kalarak toplumsal adalet için mücadele etmeye başlar.",
      'sehrın dısındakıler.jpg':
          " Hayri, İstanbul'da yaşadığı kültürel ve entelektüel çevreden uzaklaşmıştır ve şehrin dışındaki sakin ve huzurlu bir hayata çekilmiştir. Ancak kasabada da çeşitli insanlar ve olaylarla karşılaşır ve bu yeni çevresinde kendini bulmaya çalışır.",
      'Notre Dame de Paris.jpg':
          "Romanın ana karakterleri arasında Quasimodo adlı kambur bir adam, güzel ve saf Esmeralda, Prenses Phoebus ve Piskopos Claude Frollo bulunmaktadır. Hikaye, Quasimodo'nun Esmeralda'ya olan aşkını ve onun peşinde olan Frollo'nun saplantılı takibini anlatır.",
      'now it can be told.jpg':
          "İngiliz yazar Philip Gibbs'in 1920'de yayımlanan birinci Dünya Savaşı hakkındaki hatıralarını içeren bir kitaptır. Bu eser, Gibbs'in savaş sırasında gazeteci olarak yaşadığı deneyimlere dayanmaktadır.",
      'oliver twist.jpg':
          "Hikaye, Oliver'ın doğumundan yetimhaneye yerleştirilmesine, ardından kötü niyetli yetimhane görevlileri ve sokaklarda yaşayan suçlu çetelerle olan karşılaşmalarını anlatır. Oliver, sokaklarda yaşadığı zorluklara rağmen iyi kalpli ve masum bir karakterdir. ",
      'our mutual friend.jpg':
          "Roman, John Harmon adlı zengin bir mirasçının ölümü ve ardından mirasına sahip olmak isteyen insanlar arasındaki entrikalarla başlar. Bu mirası, Harmon'un vasiyeti gereği, ona iyilik yapan bir aileye bırakması beklenmektedir. Ancak Harmon, kendisini öldürmeye çalışanların elinden kaçarak başka bir kimlikle yaşamaya başlamıştır.",
      'oyle bir hikaye.jpg':
          "Sait Faik'in öyküleri genellikle İstanbul'un sokaklarında geçer ve sıradan insanların hayatlarını, duygularını ve çatışmalarını ele alır. Doğanın ve denizin insanlar üzerindeki etkilerini sıkça işler ve genellikle melankolik bir hava taşır. ",
      'poems of william blake.jpg':
          " İngiliz şair, ressam ve baskı ustası William Blake'in şiirlerini içeren bir koleksiyondur. William Blake, romantik dönemin önde gelen şairlerinden biridir ve eserleri derin düşünceler, mistisizm ve deneysel dil kullanımıyla tanınır.",
      'pride and prejudice.jpg':
          "Bennet ailesinin beş kızının evlilik arayışlarını merkeze alır. Hikaye, başlıca karakterlerden Elizabeth Bennet'in romantik ilişkileri, aşk arayışları ve toplumun beklentileriyle başa çıkma sürecini anlatır. Elizabeth, gururlu ve zengin Mr. Darcy ile tanışır ve aralarında inişli çıkışlı bir ilişki gelişir.",
      'Prince Otto, a Romance.jpg':
          "Roman, Grand Duchy of Gerolstein adlı hayali bir Alman prensliğinde geçer. Prens Otto, tahtının güvenliğini sağlamak ve ülkesini istikrara kavuşturmak için zorlu bir mücadele vermektedir. Ancak Otto'nun tahtı tehlikeye girdiğinde, kendisini ve ülkesini korumak için çeşitli politik entrikalara ve tehlikelere karşı savaşmak zorunda kalır.",
      'resurrection.jpg':
          " aristokrat bir genç olan Dmitri Nekhlyudov'un, yanlışlıkla bir kadının haksız yere hapsedilmesine neden olmasının ardından yaşadığı vicdan azabını ve bu kadına yönelik bir tür kurtarma ve kefaret çabasını anlatır. Nekhlyudov, bu kadın olan Maslova'nın dramatik dönüşümüne tanık olurken, aynı zamanda onunla bağlarını güçlendirir ve hayatındaki tüm yaşamı değiştiren bir içsel dönüşüm geçirir.",
      'Rhyme and Reason.jpg':
          "Kitap, çeşitli temalarda ve tarzlarda yazılmış şiirleri içerir. Carroll, şiirlerinde oyunbaz bir üslup kullanırken, aynı zamanda dil oyunları, mantık oyunları ve hicivli göndermelerle doludur.",
      'Roderick Hudson.jpg':
          "Roman, Amerikalı heykeltıraş Roderick Hudson'un hayatını ve kariyerini merkezine alır. Hudson, Amerika'dan İtalya'ya gider ve orada yetenekli bir sanatçı olarak tanınır. Ancak, Hudson'un kariyeri ve kişisel ilişkileri, sanatsal hırslarının ve tutkularının yanı sıra, kişisel zorluklarla da doludur.",
      'romeo-and-juliet.jpg':
          "Romeo ve Juliet, düşman iki aile olan Montague'lar ve Capulet'lerin çocuklarıdır. Ancak, bu iki genç aşık, ailelerinin düşmanlığına rağmen birbirlerine karşı derin bir tutku hissederler. Tutkulu aşklarına rağmen, trajik bir dizi olay sonucunda kendilerini bir intihar planının içinde bulurlar.",
      'rose in bloom.jpg':
          "Roman, genç ve yetenekli bir kadın olan Rose Campbell'in yaşamını konu alır. Rose, Eight Cousins romanında on dört yaşındayken tanıdığımız karakterdir. Şimdi ise yirmi bir yaşındadır ve çevresindeki insanlarla ilişkileri, aşk hayatı ve kişisel gelişimi üzerine odaklanır.",
      'saatlerı ayarlama enstıtusu.jpg':
          "Roman, İstanbul'da geçen bir günü anlatır ve birkaç farklı karakterin yaşamlarını ve ilişkilerini izler. Hikaye, Sabahattin Ali'nin ünlü öyküsü 'Kürk Mantolu Madonna' ile bağlantılıdır ve romanın ana karakterlerinden biri olan Hayri İrdal, Sabahattin Ali'nin eserlerine birkaç kez gönderme yapar.",
      'salommbo.jpg':
          "Hikaye, Kartaca'nın varlığını tehdit eden iç ve dış tehlikelerle mücadele eden bir dönemi anlatır. Kartaca'da bir isyan patlak verir ve Kartaca'nın generali Hamilcar Barca'nın kızı olan Salammbô, isyancı lider Matho'ya karşı duygusal bir bağ geliştirir.",
      'saray ve otesi.jpg':
          ". Hikaye, Osmanlı sarayının içinden ve çevresinden farklı kesimlerden kadın karakterlerin gözünden anlatılır. Sarayda yaşayan kadınlar, çeşitli sınıf ve statülerden gelirler ve her biri kendi kişisel hikayesini ve deneyimini taşır.",
      'she and allan.jpg':
          "Roman, 'She' adlı önceki romanda tanıtılan Ayesha karakterine odaklanır. Hikaye, Ayesha'nın geçmişine ve gizemli yaşamına ışık tutar.",
      'St. Ives.jpg':
          "Roman, Napolyon Savaşları sırasında Fransız bir casusluk ağına karışan St. Ives'in yaşadığı maceraları konu alır. St. Ives, esaretten kaçmak ve aşkı bulmak için çeşitli tehlikelerle dolu bir yolculuğa çıkar.",
      'Sylvie and Bruno.jpg':
          "Roman, Sylvie and Bruno adlı iki fantastik karakterin etrafında dönüyor. Sylvie, bir peri kızı olarak tanımlanırken, Bruno ise insanlarla etkileşimde bulunan hayali bir karakterdir. Hikaye, Sylvie ve Bruno'nun yaşadığı fantastik maceraları ve gerçek dünyadaki olayları iç içe geçiren karmaşık bir yapıya sahiptir.",
      'the american.jpg':
          "Romanın merkezinde, Christopher Newman adında bir Amerikalı milyarder yer almaktadır. Newman, Avrupa'da seyahat ederken Fransız aristokrat bir ailenin kızı olan Claire de Cintré ile tanışır ve ona aşık olur. Ancak, Claire'in ailesinin çıkarları ve geleneksel beklentileri, Newman ve Claire'in ilişkisinin önünde engel teşkil eder.",
      'semaver.jpg':
          "Sait Faik'in ustalıklı anlatımı ve derin gözlem yeteneği, okuyuculara sıradanlığın içinde gizlenmiş derinlikli duyguları ve insan ilişkilerini keşfetme fırsatı sunar.",
      'sense and sensibility.jpg':
          "Dashwood ailesinin iki kızı olan Elinor ve Marianne'in yaşamlarını ele alan roman, iki kızın farklı karakterlerine odaklanarak, duygusal ve rasyonel tepkiler arasındaki dengeyi inceler. Elinor, mantıklı ve sakin bir karakterken, Marianne duygusal ve tutkulu bir doğaya sahiptir. Roman, bu iki kızın aşk hayatları ve ilişkileri etrafında dönüyor.",
      'sevda sokagi komedyasi.jpg':
          "Roman, İstanbul'un farklı sosyal sınıflarından ve kültürel arka planlardan gelen karakterlerin yaşadığı Sevda Sokağı'nda geçer. Hikaye, sokaktaki komşuluk ilişkileri, aile içi çatışmalar ve aşk üzerine odaklanır. ",
      'seyehatname.jpg':
          "Evliya Çelebi Seyahatnamesi, dünya edebiyatının en eski ve en kapsamlı seyahatnamelerinden biri olarak kabul edilir. Eser, Evliya Çelebi'nin gezdiği yerlerdeki doğal güzellikler, mimari yapılar, insanlar ve kültürel detaylar hakkında ayrıntılı ve renkli bir portre sunar. ",
      'silas marner.jpg':
          "Silas Marner, insanın içsel dönüşümü, toplumsal dışlanma, ahlaki değerler ve insan ilişkileri gibi temaları ele alır. Roman, George Eliot'un derinlemesine karakter analizi ve toplumsal eleştirisiyle ünlüdür.",
      'sinekkli bakkal.jpg':
          "Roman, 19. yüzyıl sonlarında Osmanlı İmparatorluğu'nun son dönemlerinde, İstanbul'un çeşitli semtlerinden birinde bulunan Sinekli Bakkal adlı bir hanın etrafında döner. Hanın sahibi olan ve çeşitli kiracılarla ilişkilerini gözlemleyen İhsan Bey'in yaşamı ve hikayesi merkeze alınır.",
      'sipsevdi.jpg':
          "Romanın ana karakteri Meftun Bey'dir. Meftun Bey okumak için gittiği Fransa'nın Paris kentinde yıllarca kalır, fakat buraya asıl geliş sebebini unutup, eğitim görmekten kaçar.",
      'songs of innocence.jpg':
          "Şiir koleksiyonu, masumiyet ve çocukluk teması etrafında döner. Blake, çocukluğun safiyetini ve masumiyetini vurgulayarak, insanın doğası hakkında derin düşüncelere yol açar.",
      'tatarcik.jpg':
          "Hikaye, genç ve güzel bir Türk kızı olan Nigâr'ın yaşamını merkeze alır. Nigâr, Batılılaşma ve modernleşme hareketlerinin etkisi altında kalan Osmanlı toplumunda kendi kimliğini bulmaya çalışırken, aynı zamanda geleneksel değerlerle modern yaşam arasında bir denge kurmaya çalışır.",
      'tempest.jpg':
          "Prospero, bir adaya sürgün edilmiş ve burada sihirli güçlerini kullanarak adanın yerlilerini kontrol etmektedir. Oyun, Prospero'nun adaya düşmanlarından intikam alma arzusu, kızı Miranda'nın aşk hayatı ve doğaüstü varlıkların etkileşimleri etrafında döner.",
      'The Adventures of Tom Sawyer.jpg':
          "Roman, Mississippi Nehri'nin kıyısında küçük bir kasaba olan St. Petersburg'da geçer. Baş karakter olan Tom Sawyer, kasabanın yerlisi olan bir çocuktur. Tom'un maceraları, kasabanın sakin yaşamını renklendirir ve onun ve arkadaşlarının hayal gücüyle dolu dünyasına dair bir portre sunar.",
      'The Ambassadors.jpg':
          "The Ambassadors, uluslararası ilişkiler, kültürel çatışmalar, kişisel özgürlükler ve duygusal bağlılıklar gibi konuları ele alır. Roman, Amerika ile Avrupa arasındaki farklılıkları ve çelişkileri incelerken, aynı zamanda bireyin içsel çatışmalarını ve duygusal gelişimini de araştırır.",
      'The Antichrist.jpg':
          "The Antichrist, Nietzsche'nin nihilizm, güç iradesi ve aşkın insan gibi kavramları üzerine düşüncelerini içerir. Eserde, ahlaki değerlerin ve dinin insanın özgürlüğünü ve gücünü engellediğine dair derinlemesine bir eleştiri sunulur.",
      'the beautiful and damned.jpg':
          "Anthony, miras sayesinde zengin bir yaşam sürmektedir, ancak bu durum onu tembelliğe ve bohem bir hayata iter. Gloria ise güzellik ve zenginlik arayışı içindedir. Ancak, evlilikleri zamanla monotonlaşır ve hayal kırıklıklarıyla dolu bir sürece girer.",
      'The Black Arrow. A Tale Of Two Roses.jpg':
          "Kral III. Henry'nin iktidar mücadelesinde York ve Lancaster hanedanları arasındaki rekabeti konu alır. Hikaye, genç bir adam olan Dick Shelton'ın etrafında döner. Dick, ailesini öldüren siyasi bir komployu öğrenir ve ardından intikam almak için siyah bir okçuyu bulmaya karar verir.",
      'the bostanians.jpg':
          "Olive, feminist ve reformist görüşlere sahip bir kadındır ve kadın hakları hareketinin öncülerinden biridir. Romanın ana olayı, Olive'in bir kadın aktivist olan Verena Tarrant'ı etkilemeye çalışması ve onu kendi idealleri doğrultusunda şekillendirmeye çalışmasıdır.",
      'The Cossacks.jpg':
          "Roman, genç bir Rus soylusu olan Olenin'in hikayesini takip eder. Olenin, toplumunun baskıcı kurallarından ve sınırlamalarından bıkmıştır ve Kafkasya'daki Kazaklar arasında özgürlük ve macera aramaya karar verir.",
      'The Exemplary Novels of Cervantes.jpg':
          " İspanyol yazar Miguel de Cervantes'in 1613 yılında yayımlanan bir eseridir. Bu eser, on iki kısa öyküden oluşan bir novella koleksiyonudur ve Cervantes'in İspanyol edebiyatına önemli bir katkısı olarak kabul edilir.",
      'The Door in the Wall.jpg':
          "Lionel, çocukken sık sık bir duvarın içindeki gizli bir kapıyı keşfeder ve ardında muhteşem bir bahçe ve yolculuklarla dolu bir dünya bulur. Ancak, her seferinde bu fantastik dünyayı ziyaret etme şansı kısa sürer ve zamanla gerçek dünya onun üzerinde baskın gelmeye başlar.",
      'the great gatsby.jpg':
          "Gatsby, zenginlik içinde yaşayan bir adamdır ve yasak aşkı Daisy Buchanan'a yeniden ulaşma umuduyla sadece zenginlik değil, aynı zamanda sosyal statüsünü de kazanmıştır. Ancak, Gatsby'nin hayatı ve aşkı, Amerikan Rüyası'nın boşluğu ve yozlaşmasıyla çarpıcı bir şekilde karşı karşıya gelir.",
      'the brothers karamazoc.jpg':
          "Roman, Karamazov ailesinin üç oğlu Dmitri, İvan ve Alyosha'nın etrafında döner. Aile içi çatışmalar, ahlaki karmaşıklıklar, din ve varoluşsal sorgulamalar, aşk ve suç gibi temalar etrafında şekillenen karmaşık bir hikayeyi anlatır.",
      'The Burning Secret.jpg':
          "Hikaye, bir anne ve oğul arasındaki karmaşık ilişkiyi merkezine alır. Eser, bir yaz tatili sırasında, yakışıklı ve çekici bir adam olan Baron'un, bir otelde tanıştığı ve gizli bir ilişki kurmaya çalıştığı, genç bir çocuk olan Edgar'un etrafında döner.",
      'The Chartreuse of Parma.jpg':
          "Fabrice, Napolyon Savaşları döneminde yaşayan bir karakterdir ve Fransız Devrimi ve Napolyon'un yükselişi sırasında yaşanan siyasi ve toplumsal olaylarla karşı karşıya kalır. Fabrice'nin tutkulu aşkı ve macera dolu hayatı, romanın ana olay örgüsünü oluşturur.",
      'the confidence.jpg':
          "bir yolcu gemisinde gecen romanda  çeşitli karakterlerin etkileşimlerini ve tartışmalarını anlatır. karakterler arasinda sürekli kimlik değiştiren bir dolandırıcı olan Güven Adamı da bulunmaktadır. dolandiricinin çeşitli kılıklar da nasıl hareket ettiğini ve karakterleri nasıl etkilediğini gösterirken, bir yandan Amerikan toplumunun çeşitli yönlerine eleştirel bir bakış sunar.",
      'the crime and punishment.jpg':
          "Raskolnikov, yoksulluk içinde yaşayan bir hukuk öğrencisidir ve toplumun üst sınıfıyla ilgili karmaşık düşüncelere sahiptir. Bu düşünceler onu, zengin bir tefeci kadını öldürme planı yapmaya iter.",
      'The Crime of the Congo.jpg':
          " İrlandalı gazeteci ve yazar Sir Arthur Conan Doyle'un 1909'da yayımlanan bir kitabıdır. Bu eser, o dönemde Kongo'da Belçika Kralı II. Leopold'un idaresi altında yaşanan kölelik ve sömürgecilik suçlarına dikkat çekmek için kaleme alınmıştır.",
      'The Critique of Pure Reason.jpg':
          "Kitap, insan bilgisinin sınırlarını ve bilgi edinme sürecini araştıran bir eleştirel inceleme sunar. Kant, deneyimin sınırlarını ve akıl yoluyla ulaşılabilecek bilginin doğasını tartışırken, rasyonel düşüncenin ve aklın olanaklarını araştırır.",
      'the desert village.jpg':
          "Şiir, bir İrlanda köyünün terk edilmiş halini ve kırsal yaşamın yıkımını anlatır. Goldsmith, modernleşmenin etkileri ve kırsal yaşamın çöküşü üzerine derin bir eleştiri sunar.",
      'The First Men In The Moon.jpg':
          "Roman, iki ana karakter olan Bedford ve Cavor'un birlikte Ay'a yapacakları fantastik bir yolculuğu konu alır. Cavor, dahi bir bilim insanıdır ve Ay'da keşif yapmak için bir uzay gemisi icat etmiştir. Bedford, Cavor'un projesine katılır ve ikili birlikte Ay'a doğru maceralı bir yolculuğa çıkarlar.",
      'The Food of the Gods.jpg':
          "Roman, iki ana karakter olan Mr. Bensington ve Mr. Redwood'un etrafında döner. İkili, bir büyüme hormonu keşfeder ve bu hormonun hayvanlar üzerindeki etkilerini test etmeye karar verir. Ancak, bu deneyler beklenmedik bir şekilde kontrolden çıkar ve keşfedilen besin maddesi, normalden çok daha hızlı büyüyen ve devasa boyutlara ulaşan hayvanlar yaratır.",
      'The Fur Country.jpg':
          "bir grup cesur maceracının, Hudson Körfezi'ndeki soğuk ve zorlu koşullarda bir kürk ticareti istasyonu kurma girişimini konu alır. kutup keşifleri, yerli halkla karşılaşmalar ve vahşi doğanın zorluklarıyla başa çıkmak zorunda iken, aynı zamanda doğal kaynakların korunması ve ticaretin sürdürülebilirliği gibi konuları da ele alırlar.",
      'the gambler.jpg':
          "Alexei, kumar oynama tutkusu yüzünden aşkını ve hatta maddi varlığını riske atar. Roman boyunca, Alexei'nin kumarhane maceraları ve duygusal çalkantıları anlatılırken, aynı zamanda toplumsal sınıf farkları ve insan doğası üzerine derinlemesine bir analiz sunulur.",
      'the genealogy of morals.jpg':
          " Alman filozof Friedrich Nietzsche'nin 1887'de yayımlanan bir eseridir. Bu kitap, Nietzsche'nin ahlaki değerlerin kökeni ve evrimi üzerine derinlemesine bir analiz sunar.",
      'the gold bug.jpg':
          "Legrand, Virginia'da yaşayan bir adamdır ve bir gün yalnızca bir böceğin izini takip ederek bir hazine haritası bulur. Bu harita, bir zamanlar korsanların kullanmış olduğu gizli bir hazineyi işaret etmektedir.",
      'the golden bowl.jpg':
          "Roman, Amerikalı sanat koleksiyoncusu Adam Verver ve kızı Maggie Verver'in İngiliz soylusu Prince Amerigo ile evliliği etrafında döner. Ancak, hikaye, Prince Amerigo'nun evli olduğu başka bir kadına, Charlotte Stant'a olan tutkusunu ve bu tutkunun ortaya çıkardığı karmaşık ilişkileri de içerir.",
      'The great boer war.jpg':
          "İngiliz İmparatorluğu ile Boer Cumhuriyetleri arasındaki çatışmaları, askeri stratejileri ve çeşitli savaş alanlarını ele alan bu kitap ayrıca, savaşın sosyal, politik ve askeri sonuçlarını da tartışır.",
      'The History Of Mr. Polly.jpg':
          "Alfred, bir dizi olayın ardından işini bırakır ve farklı maceralar aramaya başlar. Ancak, kaçış girişimleri beklenmedik sonuçlar doğurur ve Alfred, kendi hayatını yeniden değerlendirmek zorunda kalır. Sonunda, kendi mutluluğunu bulmak için içsel bir yolculuğa çıkar.",
      'The Hunting of the Snark.jpg':
          "Kitap, bir grup seyahatçının, gizemli ve hayali bir yaratık olan Snark'ı avlamak için çıktıkları macerayı anlatır. Seçkin bir grup, liderleri Bellman'ın komutası altında, Snark'ın peşine düşer ve çeşitli fantastik ve tuhaf maceralarla karşılaşır.",
      'The Innocents Abroad, travel.jpg':
          "Mark Twain dönemin Avrupa ve Ortadoğu toplumlarına ve kültürlerine dair eğlenceli ve bazen de alaycı bir bakış sunar. Twain'in mizah anlayışı ve hiciv yeteneği, okuyucuları hem güldürürken hem de düşündürürken, seyahat yazısı türünün önemli bir örneğini oluşturur.",
      'the light.jpg':
          "Fransız asker olan Jean Diaz'ın gözünden savaşı anlatır. Diaz, cephe hattında diğer askerlerle birlikte savaşırken, savaşın korkunç gerçekleriyle yüzleşir. Savaşın anlamsızlığı, ölüm korkusu ve insanın en kötü yanlarının ortaya çıkması, Diaz ve diğer askerlerin deneyimlediği şeyler arasındadır.",
      'the history of england i.jpg':
          "Hume'un tarih eseri, o dönemin çeşitli siyasi olaylarını, monarşiyi, parlamentoyu, dini çatışmaları ve İngiltere'nin genel gelişimini ele alır. Ancak, Hume'un tarih anlayışı modern tarihçilerinki gibi detaylı ve belge odaklı değildir.",
      'the history of england ii.jpg':
          "Hume'un tarih eseri, o dönemin çeşitli siyasi olaylarını, monarşiyi, parlamentoyu, dini çatışmaları ve İngiltere'nin genel gelişimini ele alır. Ancak, Hume'un tarih anlayışı modern tarihçilerinki gibi detaylı ve belge odaklı değildir.",
      'the history of england iii.jpg':
          "Hume'un tarih eseri, o dönemin çeşitli siyasi olaylarını, monarşiyi, parlamentoyu, dini çatışmaları ve İngiltere'nin genel gelişimini ele alır. Ancak, Hume'un tarih anlayışı modern tarihçilerinki gibi detaylı ve belge odaklı değildir.",
      'the history of england iv.jpg':
          "Hume'un tarih eseri, o dönemin çeşitli siyasi olaylarını, monarşiyi, parlamentoyu, dini çatışmaları ve İngiltere'nin genel gelişimini ele alır. Ancak, Hume'un tarih anlayışı modern tarihçilerinki gibi detaylı ve belge odaklı değildir. ",
      'the history of england v.jpg':
          "Hume'un tarih eseri, o dönemin çeşitli siyasi olaylarını, monarşiyi, parlamentoyu, dini çatışmaları ve İngiltere'nin genel gelişimini ele alır. Ancak, Hume'un tarih anlayışı modern tarihçilerinki gibi detaylı ve belge odaklı değildir.",
      'the history of england vi.jpg':
          "Hume'un tarih eseri, o dönemin çeşitli siyasi olaylarını, monarşiyi, parlamentoyu, dini çatışmaları ve İngiltere'nin genel gelişimini ele alır. Ancak, Hume'un tarih anlayışı modern tarihçilerinki gibi detaylı ve belge odaklı değildir.",
      'the idiot.jpg':
          "Prens Lev Nikolayeviç Myshkin'in hikayesini anlatır. Myshkin, zihinsel bir rahatsızlık nedeniyle uzun bir süre İsviçre'de tedavi gördükten sonra Rusya'ya döner. Ancak, sosyal ortamında ve çevresindeki insanlar arasında yaşadığı zorluklarla karşılaşır.",
      'the inferno.jpg':
          "savaşın cehennemine doğru bir yolculuğa çıkan bir grup askerin deneyimlerini konu alır. Askerler, savaşın korkunç gerçekleriyle yüzleşirken, insan doğasının karanlık yönlerini de keşfederler.",
      'The Invisible Man.jpg':
          "Roman, bir bilim adamı olan Griffin'in, kendisini görünmez kılacak bir formül geliştirmesiyle başlar. Ancak, bu deney sonrasında görünmezlik yeteneği, Griffin'in hayatını tamamen değiştirir ve onu kontrol edilemez bir hal alır.",
      'The Island of Dr. Moreau.jpg':
          "Denizci Edward bir gemi kazasi sonucu Dr.Moreau'nun adasina suruklenir.Bu adada, bir bilim insanı, hayvanlar üzerinde insan genleriyle oynayarak yürüttüğü korkunç deneylerle uğraşmaktadır. Moreau'nun yarattığı yarı insan yarı hayvan yaratıklar, adanın gerilim dolu atmosferini oluşturur.",
      'the journal to stella.jpg':
          "Esther Johnson ile olan ilişkisi, Swift'in hayatının önemli bir parçasını oluşturur. Mektuplarda, Swift'in kişisel düşünceleri, duyguları ve dönemin siyasi olaylarına dair yorumları bulunur. Mektuplar aracılığıyla Swift, Stella'ya sık sık onu düşündüğünü ve hissetiklerini ifade eder.",
      'the king solomon.jpg':
          "Quatermain, bir grup maceraperestle birlikte Afrika'nın derinliklerine doğru tehlikeli bir yolculuğa çıkar. Macera dolu bu yolculukta, kahramanlar çeşitli tehlikelerle karşılaşırken, efsanevi zenginlikleri aramaktadırlar.",
      'The Man in the Brown Suits.jpg':
          "Anne, Londra'daki sıradan yaşamından sıkılmıştır ve hayatına heyecan katmak için Afrika'ya gitmeye karar verir. Bir gemi yolculuğu sırasında tanıştığı gizemli bir adamın ölümüyle başlayan olaylar, Anne'i bir dizi tehlikeli olayın içine sürükler.",
      'the masque of the red death.jpg':
          "Hikaye, kırmızı ölüm adında ölümcül salgının hüküm sürdüğü ülkede,  zengin bir soylu olan Prens Prospero , salgından korunmak için kendisine ait kalede izole bir hayat yaşamaya karar verir. Kalede bir maskeli balo düzenler ve davetlileri, salgından korunmak için dış dünyadan soyutlanmış bir şekilde eğlenirler. Ancak, bir gece balo da, bir figür ortaya çıkar: Kızıl Ölüm'ün kendisi.",
      'The Murder on the links.jpg':
          "Hikaye, zengin bir Fransız adam olan Renauld'un öldürülmesiyle başlar. Olay yerine gelen Hercule Poirot, cinayeti çözmek için harekete geçer. Renauld'un ölümüyle birlikte, çeşitli şüpheler ve gizemler ortaya çıkar. ",
      'The Mysterious Island.jpg':
          "Amerikan İç Savaşı döneminde, bir grup kaçak tarafından savaştan kaçmak için kullanılan balonun, kasırga nedeniyle sürüklenmesi sonucu bir adaya düşmesiyle başlar. Balonun yolcuları, Güney Pasifik'teki bu gizemli adada hayatta kalmaya ve adanın sırlarını keşfetmeye çalışırlar.",
      'the portrait of dorian gray.jpg':
          "Baş karakter Dorian Gray, yakışıklı ve genç bir adamdır. Bir ressam olan Basil Hallward, Dorian'ın portresini çizerken, onun güzelliği ve gençliği karşısında büyülenir. Dorian, Lord Henry Wotton adında bir arkadaşı tarafından etkilenir ve hedonistik bir yaşam tarzını benimser.",
      'The Prince and the Pauper.jpg':
          "Prens Edward, İngiltere Kralı Henry VIII'in oğludur ve lüks bir yaşam sürmektedir. Tom Canty ise fakir bir ailenin çocuğudur ve sarayın yakınında yaşamaktadır. Bir gün, ikisi de birbirine benzeyen fiziksel özelliklere sahip olduklarını keşfederler ve birbirlerinin yerine geçmeye karar verirler. Ancak, bu beklenmedik sonuçlar doğurur ve her ikisinin de hayatı büyük ölçüde değişir.",
      'The Sea Lady.jpg':
          "Romanın ana karakteri, denizkızı olarak bilinen bir deniz yaratığı olan Nettie, insanları daha yakından tanımak için denizden çıkıp insanların arasına katılmaya karar verir. Ancak, insan toplumunda var olmak ve kabul görmek, beklediği kadar kolay olmayacaktır.",
      'The Time Machine.jpg':
          "Bir bilimadamı ve mucit olan bir karakterin, Zaman Makinesi adını verdiği bir icadıyla zaman içinde yolculuk yapma yeteneğini keşfetmesiyle başlar. Zaman Makinesi'ni kullanarak geleceğe doğru bir yolculuğa çıkar. Gelecekte, dünya üzerinde farklı insan türlerinin evrim geçirmiş olduğunu keşfeder.",
      'the mystery marie roget.jpg':
          " Marie Rogêt'nin gizemli bir şekilde ortadan kaybolması ve ardından ölü bedeni bulunmasıyla başlar. Cinayetin çözülmesi için polis, basın ve halk büyük bir ilgi gösterir, ancak gizem çözülemez. Bunun üzerine, C. Auguste Dupin, karmaşık ipuçlarını ve delilleri incelemeye karar verir.",
      'the mystery of edwin drood.jpg':
          "Drood'un kaybolması, okuyucuları ve diğer karakterleri endişelendirir ve Drood'un nişanlısı olan Rosa Bud'un da içinde bulunduğu bir dizi şüpheli ortaya çıkar. Romanın sonuna yaklaşılırken, gizemli olayların çözülmesi beklenirken, Dickens'ın ani ölümü nedeniyle hikaye tamamlanmamıştır.",
      'the mystery of the blue train.jpg':
          "Hikaye, mavi bir trende yaşanan bir cinayetle başlar. Tren yolcularından biri olan Ruth Kettering'in cesedi, Poirot'un dikkatini çeker ve cinayeti çözmeye çalışır. Cinayetin ardında büyük miktarda bir mirasın olduğu ortaya çıkar ve olaylar, bir dizi karmaşık ipucu ve şüpheli ile gelişir.",
      'The New Machiavelli.jpg':
          "Remington, siyasi güç ve başarıya ulaşmak için her türlü yolu denemeye kararlıdır. Ancak, bu hırsı ve tutkusu, kişisel ve ahlaki zorluklarla karşılaşmasına neden olur. Remington, Machiavelli'nin prensiplerini takip ederek politik arenada yükselmeye çalışırken, kendi içsel çatışmalarıyla da mücadele etmek zorundadır.",
      'the prince.jpg':
          "Machiavelli, kitapta hükümdarlara, güçlerini korumak ve genişletmek için her türlü yöntemi kullanmalarını tavsiye eder. İdealist ve ahlaki prensiplerin gerçek dünyada yetersiz kaldığını savunur ve politikanın pragmatik bir yaklaşım gerektirdiğini öne sürer.",
      'the raven.jpg':
          "Hikaye, bir kış gecesi bir adamın, yaşadığı yas ve depresyonun etkisi altında yalnızlık içinde oturduğu odada bir ziyaretçinin gelmesiyle başlar. Ziyaretçi, bir karga olan Raven'dır. Karga, insanın ruhunu sorgular ve adamın iç dünyasını yansıtır.",
      'The Red and The Black.jpg':
          "Julien, bir papazın oğlu olarak düşük bir sosyal sınıftan gelir, ancak kendi hayatını şekillendirme arzusu ve zekasıyla öne çıkar. Roman, Julien'in zengin bir ailenin konağında öğretmenlik yapmaya başlamasıyla başlar ve ardından yükselme arzusu ve aşk ilişkileriyle dolu bir yolculuğa çıkar.",
      'The Research Magnificent.jpg':
          "Porphyry, bilgiye olan merakıyla tanınan biridir ve insanlığın geleceği hakkında derin düşüncelere sahiptir. Roman, Porphyry'nin hayatındaki dönüm noktalarını ve içsel sorgulamalarını takip ederken, bilimin ve teknolojinin insanlığı nasıl etkilediğini ve değiştirdiğini inceler.",
      'The Secret Adversary.jpg':
          "1919 Londra'sında, terhis olmuş asker Tommy Beresford, savaş gönüllüsü Prudence, Tuppence Cowley ile tanışır. İkisi de işsiz ve beş parasızdır. 'Young Adventurers, Ltd.' şirketini kurarlar. Bay Whittington, Tuppence'ı çalışmalarını sunması için takip eder,",
      'The sleeper awakes.jpg':
          "Graham, bir hastalık nedeniyle uzun bir uykuya dalar ve yaklaşık iki yüz yıl sonra uyanır. Uyandığında, dünya büyük ölçüde değişmiştir; insanlar, büyük şehirlerde yaşamakta ve toplum, devasa bir şirket tarafından yönetilmektedir.",
      'the soul of a nation.jpg':
          " Gibbs, kitabında Birinci Dünya Savaşı sırasında İngiltere'nin ruh halini ve toplumsal atmosferini inceler. Savaşın etkileri, halkın duygusal tepkileri ve savaşın toplumdaki derin etkileri gibi konular ele alınır.",
      'The Thomas Jefferson writes.jpg':
          " Bu eserler, Amerikan tarihindeki belirli olaylarla ilgili Jefferson'ın görüşlerini, siyasi fikirlerini ve ideallerini ortaya koyar. Ayrıca, Jefferson'ın kişisel düşünceleri ve duyguları da bu yazılı eserlerde bulunabilir.",
      'the trial.jpg':
          "Hikaye, baş karakter Josef K.'nın beklenmedik bir şekilde tutuklanması ve ardından yargılanmasıyla başlar. Ancak, K. neyle suçlandığını tam olarak bilmez ve sürecin neden olduğunu anlamaya çalışırken kendini karmaşık bir hukuki sistemin içinde bulur.",
      'The Twilight of the Idols.jpg':
          "Kitap, Nietzsche'nin diğer eserlerinde ele aldığı birçok temayı genişletir ve derinleştirir. Nietzsche,  ahlaki değerlerin, dinin ve kültürel normların eleştirisini yapar. Kitap, Batı medeniyetinin temellerini sorgular ve insanlığın değerlerinin çöküşünü tartışır.",
      'the voyage of the beagle.jpg':
          "Kitap, Darwin'in dünya çapında farklı coğrafi bölgelerde yaptığı araştırmaları, gözlemleri ve keşifleri anlatır. Darwin, Güney Amerika, Galapagos Adaları, Avustralya ve Yeni Zelanda gibi bölgelerde birçok farklı türü incelemiş ve farklı türler arasındaki ilişkileri gözlemlemiştir.",
      'the voyage out.jpg':
          "Rachel Vinrace'in İngiltere'den Güney Amerika'ya yapılan bir deniz yolculuğuna katılir. Rachel, yolculuk sırasında farklı insanlarla tanışır ve çeşitli deneyimler yaşar. Yolculuk boyunca, Rachel kendi kimliğini ve yerini bulmaya çalışırken, çevresindeki insanların ilişkileri ve toplumsal normlar hakkında da derin düşüncelere dalar.",
      'The War in the Air.jpg':
          "Bir ingiliz subayi bindigi hava balonunda tanistigi alman bilim adami tarafindan icat edilen devrim niteliginde bir savas ucagina rastlar. Ucak kullanilmak icin calinir ve dünya çapında bir savaşın patlak vermesine neden olur.",
      'The war in the east japan.jpg':
          "Evans ve Peattie, Japon İmparatorluk Ordusu'nun çeşitli savaş stratejilerini, operasyonlarını ve askeri harekatlarını detaylı bir şekilde inceler. Kitap, Japon ordusunun Çin, Kore, Mançurya ve diğer Doğu Asya ülkelerine yönelik saldırılarını ve işgalini açıklar.",
      'the war of the worlds.jpg':
          "Hikaye, Marslıların Dünya'ya ani bir saldırısıyla başlar. Marslılar, ünlü üç bacaklı makinelerini ve ölümcül ısı raylarını kullanarak insanları ve şehirleri yok eder. Romanın ana karakteri, ismi verilmemiş bir İngiliz adamıdır, olayları yaşadığı yerden anlatır.",
      'The Wheels of Chance.jpg':
          "Hikaye, bir bisiklet tamircisi olan Hoopdriver'ın, izinsiz bir şekilde bisikletle bir maceraya atılmasını konu alır. Hoopdriver, sıradan bir yaşam sürerken, tesadüfen tanıştığı ve aşık olduğu bir kadınla birlikte romantik bir yolculuğa çıkar.",
      'the whıte company.jpg':
          "Hikaye, genç ve cesur bir İngiliz askeri olan Alleyne Edricson'un maceralarını takip eder. Alleyne, kendisini adalete ve şövalyeliğe adamış bir grup savaşçıya, The White Company'e katılır. Bu grup, Fransızlarla savaşan İngiliz ordusuna katılmak üzere yola çıkar.",
      'The wizard of wall street and his wealth.jpg':
          "Bu kitap, 20. yüzyılın başında ABD'nin finans dünyasında büyük etkiye sahip olan John D. Rockefeller'ın hayatını ve servetini anlatır. Kitap, Rockefeller'ın doğumundan itibaren hayatını, iş hayatındaki yükselişini ve petrol endüstrisindeki liderliğini detaylı bir şekilde inceler.",
      'The Wonderful Visit.jpg':
          "Hikaye, İngiltere'de sıradan bir kasabada geçerken, bir melek olan Ariel'in beklenmedik bir ziyaretiyle başlar. Ariel, bir melek olarak dünyada gezinirken, kasabada bir dizi ilginç olaya neden olur ve insanlar üzerinde derin etkiler bırakır.",
      'The World Set Free.jpg':
          "Hikaye, 20. yüzyılın başlarında geçerken, insanlık nükleer enerjiyi keşfeder ve bunu silah olarak kullanır. Bu silahlar, şehirleri yok eden ve dünyayı değiştiren yıkıcı güçlere sahiptir.",
      'the wuthering heights.jpg':
          "Hikaye, Heathcliff adında gizemli bir yetim çocuğun, Earnshaw ailesi tarafından yetiştirilmesiyle başlar. Heathcliff, kaba ve sert bir çocuk olarak tanıtılır, ancak zamanla ailenin kızı Catherine Earnshaw'a olan derin aşkı hikayenin merkezine yerleşir.",
      'the-sorrows-of-young-werther.jpg':
          "Roman, genç ve duygusal Werther'in mektupları aracılığıyla okuyucuya sunulur. Werther, güzel Charlotte'e olan aşkını anlatırken, içinde bulunduğu acıları ve çatışmaları da ifade eder. Ancak, Charlotte'in başka bir adamla evlenme kararı almistir.",
      'this side of paradise.jpg':
          "Roman, Princeton Üniversitesi'nde okuyan genç ve zengin Amory Blaine'in yaşamını anlatır. Amory'nin eğitimi, ilişkileri ve kişisel gelişimi üzerine odaklanan hikaye, Amerikan gençliğinin ve toplumunun döneminin bir portresini çizer.",
      'Three Sunsets and Other Poems.jpg':
          "Carroll'ın mizahi ve ironik üslubu, bu şiirlerde de kendini gösterir. Three Sunsets adlı şiir, doğa ve insan duyguları üzerine derinlemesine bir düşünce sunarken, diğer şiirler de farklı konuları ele alır.",
      'three-essays-on-the-theory-of-sexuality.jpg':
          "Kitapta, Freud cinselliğin doğası, gelişimi ve insan psikolojisi üzerindeki etkileri üzerine kapsamlı bir analiz sunar. Freud, cinsel dürtülerin çocuklukta nasıl geliştiğini, cinsel kimlik ve cinsel yaşamın önemini ve cinselliğin toplumsal ve kültürel etkilerini inceler.",
      'Tom Sawyer Abroad.jpg':
          "Hikaye, Tom, Huck ve Jim'in bir zeplinle dünya turuna çıkmalarıyla başlar. Ancak, zeplin rotasından sapar ve uzak bir yerde, tanıdık olmayan bir ülkede bir adaya inerler. Burada, tuhaf ve komik olaylarla karşılaşırlar ve adanın yerlileriyle etkileşime girerler.",
      'Tom Sawyer, Detective.jpg':
          "Tom ve Huck'ın, eski bir köle olan Jim'in kaybolan bir defineyi bulmak için maceraya atilir. İkilinin izleyebileceği tek kişi olan Jim, önceleri bir hırsızlık suçlaması nedeniyle kaçmıştır. Tom ve Huck, Jim'in masumiyetini kanıtlamak için iz sürerler ve bu süreçte bir dizi macera yaşarlar.",
      'Tono-Bungay.jpg':
          "Roman, bir tür tılsım ve panzehir olan Tono-Bungay adlı mucizevi bir ilacın yaratılmasını ve bu ilacın etrafında dönen olayları konu alır. Başlangıçta, bu ilaç tüccarının yeğeni George Ponderevo'nun çocukluk anılarıyla başlar ve daha sonra George'un yetişkinlik dönemine geçer.",
      'totem-and-taboo.jpg':
          "Totemizm, bir grup insanın kendilerini bir hayvana veya nesneye bağlı hissettiği ve bu nesneyi kutsal kabul ettiği bir inanç sistemidir. Tabu ise toplumun belirli davranışları yasaklaması ve bu yasakları kutsal saymasıdır.",
      'thus spake zarathustra.jpg':
          "Zerdüşt, insanlığa yeni bir ahlak ve değerler sistemi getirmek amacıyla dağlara çekilir ve orada on yıl boyunca düşünür. Sonunda, insanlara 'Übermensch' (ötesi-insan) fikrini getirir ve geleneksel ahlaki değerlerin ötesine geçmeyi önerir.",
      'the white nights.jpg':
          "Roman, genç bir adamın St. Petersburg'da tanıştığı ve aşık olduğu bir genç kadınla geçirdiği dört beyaz gecenin hikayesini konu alır. Genç adam, gizemli ve melankolik bir kadın olan Nastenka ile tanışır ve ona hislerini açar. Ancak, Nastenka'nın hayatında başka bir adam vardir.",
      'turkun atesle imtihani.jpg':
          "Kitap, milli mücadelenin çeşitli aşamalarını ve bu süreçte yaşanan kahramanlıkları ele alır. Ana karakterler arasında, çeşitli sosyal ve ekonomik arka planlardan gelen gençlerin yer aldığı bir grup vardır.",
      'Twenty Thousand Leagues Under the Sea.jpg':
          "Roman, Nautilus adlı bir denizaltıda seyahat eden Profesör Aronnax, denizbiyoloğu Conseil ve balina avcısı Ned Land'in maceralarını anlatır. Nautilus'un kaptanı, gizemli bir deniz adamı olan Kaptan Nemo'dur.",
      'Under Sentence of Death.jpg':
          "Öykü, denizaltı mürettebatının bir gemi batırma operasyonunda yakalanmasının ardından yaşadıkları sonu gelmez maceraları ele alır. Mürettebat, gemi batırma faaliyetlerinin uluslararası hukuka aykırı olması nedeniyle ölüm cezasına çarptırılır ve denizaltının içinde mahkûm edilirler.",
      'Vicar of Wakefield.jpg':
          " Rahip Primrose, uysal ve erdemli bir karakterdir ve ailesiyle birlikte sakin bir yaşam sürmektedir. Ancak bir dizi şanssız olay sonucunda ailesi zor zamanlar geçirmeye başlar.",
      'vurun kahpeye.jpg':
          "Romanda, idealist İstanbullu öğretmen Aliye’nin Anadolu’da bir kasabaya gidişi ve bölgede Milli Mücadele düşüncesine destek faaliyetleri aktarılır.",
      'white bird.jpg':
          " James, Elspeth adında bir kadına derinden âşık olmuştur. Ancak, Elspeth zaten denizde olan bir adam olan David Webster ile nişanlanmıştır. Buna rağmen, James Elspeth'in sonunda onu seçeceğini umut etmeye devam eder.",
      'Wilhelm Meister.jpg':
          "Wilhelm Meister, hayatını keşfetme ve kendini gerçekleştirme arayışında olan bir gençtir. İşleyici bir aileden gelmesine rağmen, sanat ve tiyatro dünyasına ilgi duyar.",
      'yasadigim gibi.jpg':
          "Kitap, Tanpınar'ın çocukluk yıllarından başlayarak yetişkinliğine kadar olan dönemi kapsar. Yazar, ailesi, eğitimi, edebi ve entelektüel merakları, çevresiyle ilişkileri ve edebiyat dünyasındaki deneyimleri hakkında okuyucuya içten bir şekilde bilgi verir.",
      'yeni dunya.jpg':
          "13 hikâyesinden oluşan Kitap yazarın hikâye alanında önemli bir yetkinliğe ulaştığının göstergesi olması bakından önemlidir. İçerik olarak kasaba ve köy yaşamı dışında düşkün kadınları ele alan hikâyeler önemli bir yer tutar.",
      'yezidin kizi.jpg':
          "bir gemi seyahati sırasında başlayıp uçsuz bucaksız Suriye çöllerinde devam eden gizemli bir aşk...",
      'zadig.jpg':
          " Zadig, hayatının çeşitli dönemlerinde felaketlerle karşılaşır ve bu felaketler onun karakterini şekillendirir. Zadig, hikaye boyunca bilgelik, adalet ve doğruluk gibi kavramları araştırır ve bu süreçte farklı insanlarla karşılaşır.",
      'A Christmas Carol.jpg':
          "Hikaye, Scrooge'un Noel gecesi eski ortağı Jacob Marley'in hayaleti tarafından ziyaret edilmesiyle başlar. Marley, Scrooge'u kendi bencillik ve sevgisizlik dolu yaşamının sonuçları konusunda uyarmak için gelir ve Scrooge'u üç hayaletin ziyaret edeceği bir yolculuğa çıkmaya ikna eder.",
      'A Modern Utopia.jpg':
          "Roman, en çok, Samuray olarak bilinen gönüllü bir soylular sınıfının statik değil kinetik bir dünya devletini etkili bir şekilde yönetebileceği ve böylece ilerlemeyi siyasi istikrarla birleştirme sorununu çözebileceği fikriyle tanınır.",
      'a plug year.jpg':
          "Daniel Defoe tarafından Londra'daki büyük veba salgını sırasında yaşananları anlatan bir günlük formunda kaleme alınmıştır. Salgının şehre yayılması, insanların karşılaştığı zorluklar, hastalığın yayılma şekilleri ve toplumun bu trajediye tepkileri gibi konular eserde detaylı bir şekilde ele alınır.",
      'america.jpg':
          "Hikaye, genç bir adam olan Karl Rossmann'ın Amerika'ya gönderilmesiyle başlar. Karl, ailesinin utanç verici bir olay nedeniyle Avrupa'dan sürülür ve Amerika'ya gitmek zorunda kalır.",
      'animal farm.jpg':
          "Hikaye, bir çiftlikte yaşayan hayvanların ayaklanması ve çiftliği ele geçirerek insan yönetimini devirmesiyle başlar. Ancak, zamanla çiftliğin yönetimi, domuzlar önderliğindeki hayvanlar arasında bir hiyerarşi oluşturulur ve çiftlikteki güç kavgaları başlar.",
      'anna karenina.jpg':
          " Anna'nın mutsuz bir evliliğe sahip olmasını ve ardından genç ve karizmatik subay Kont Vronsky ile olan yasak aşkını konu alır. Anna'nın aşkı ve toplum tarafından dışlanması, onun ruhsal çöküşüne yol açar ve trajik bir sonla sonuçlanır.",
      'A London Life.jpg':
          "Konu, çökmekte olan bir evlilik ve bunun birçok insan, özellikle de yakında boşanacak olan karısının kız kardeşi Laura Wing üzerindeki etkisi etrafında döner.",
      'art of war.jpg':
          "Sun Tzu'nun Savaş Sanatı ,  Çinli askeri stratejist Sun Tzu tarafından yazılan klasik bir metindir. MÖ 5. yüzyıla tarihlenen bu eser, askeri strateji ve liderlik üzerine derinlemesine bilgiler içerir.",
      'ayesha the return of she.jpg':
          "Ayesha, The Return of She, İngiliz Viktorya dönemi yazarı H. Rider Haggard'ın 1905'te She'nin devamı olarak yayınlanan gotik-fantastik romanıdır. Kronolojik olarak Ayesha ve Allan Quatermain serisinin son romanıdır.",
      'cadi.jpg':
          "dul Naşit Nefi Efendi yeniden evlenmiştir ve evinin yalısında cadılar dolaşmaya başlamıştır. Naşit Nefi Efendi'nin ilk eşi Binnaz Hanım'ın cadı olarak geri döndüğü, Naşit Nefi Efendi ve eşlerine musallat olarak onlara rahat vermediği gibi olaylar zincirinde çıkmaza girilir.",
      'boon.jpg':
          "Boon, bir grup entelektüel ve düşünürün, insanlığın ilerlemesi ve evrimsel yolculuğu üzerine yaptığı tartışmaları konu alır.",
      'ask batagi.jpg':
          "ailesinin şımarık bir şekilde yetiştirdiği ayran gönüllü sürekli kadınlardan sıkılan iki kez evlilik yapmış Naki Efendi'nin üçüncü eşi Bedia ile olan evliliğini anlatmış.Eski İstanbul yaşayışı , aşkları, iliskilerin ne derece bozuk olduğunu gözler önüne sermiş.",
      'Anticipations.jpg':
          "Wells, Anticipations'da, 20. yüzyılın başlarında toplumsal ve teknolojik gelişmeleri, siyasi değişimleri ve insanlığın geleceğini nasıl şekillendirebileceğini tartışır.",
      'aski memnu.jpg':
          "Bihter, kendisinden yaşça büyük Adnan Bey ile evlendirilmiş, ancak daha sonra aşık olduğu Behlül ile yasak bir ilişki yaşamaya başlamıştır. Diğer taraftan, Adnan Bey'in yeğeni olan Nihal, Behlül'e platonik bir aşk beslemektedir.",
      'bealby.jpg':
          "Romanın baş karakteri olan Bealby, kendisini istemediği bir aile ortamından kurtarmak için kaçar ve bir otelde iş bulur. Ancak, burada da kendisini bir dizi olayın içinde bulur ve beklenmedik maceralara atılır.",
      'kipps.jpg':
          "Kipps, küçük bir kasabada bir terzi çıraklığı yaparken ansızın miras kalan bir servetle zengin bir adam haline gelir. Bu servet sayesinde daha yüksek sosyal sınıflara yükselir ve İngiliz toplumunun çeşitli kesimleriyle tanışır.",
      'kirik hayatlar.jpg':
          "Kitaptaki olaylar Osmanlı’nın son döneminde geçmektedir. Osmanlı’nın son döneminde Türk halkında batıya karşı körü körüne bir özenti oluşur. Batıdan alınması gereken teknoloji, ilim, bilim değilde; bizim yaşantımıza ters düşen kültürü taklit edilir.",
      'Italian Hours.jpg':
          "Kitaptaki olaylar Osmanlı’nın son döneminde geçmektedir. Osmanlı’nın son döneminde Türk halkında batıya karşı körü körüne bir özenti oluşur. Batıdan alınması gereken teknoloji, ilim, bilim değilde; bizim yaşantımıza ters düşen kültürü taklit edilir.",
      'little dorrit.jpg':
          "Romanın ana karakteri olan Amy Dorrit, London'daki Marshalsea Hapishanesi'nde doğup büyümüş bir genç kızdır. Babası, borçları nedeniyle hapis hayatı yaşayan bir adamdır.",
      'little men.jpg':
          "Hikaye, Little Women'da tanıtılan March ailesinin üyelerinden biri olan Jo March'ın yaşamını sürdürdüğü Plumfield adlı okulun etrafında döner. Jo ve kocası Professor Bhaer, Plumfield adlı bir çocuk okulu açarlar ve bu okulda farklı yaşlardaki çocuklar bir arada eğitim görürler.",
      'little woman.jpg':
          "Ana karakterlerden biri olan Jo March ve üç kız kardeşi, Amerikan İç Savaşı döneminde yaşayan sıradan bir aileden gelirler. Roman, kız kardeşlerin ergenlikten yetişkinliğe geçiş sürecinde yaşadıkları zorlukları, sevinçleri, aşkları ve hayal kırıklıklarını konu alır.",
      'meditations.jpg':
          "Marcus Aurelius, Roma İmparatorluğu'nun bir hükümdarı olarak günlük yaşamın getirdiği zorluklarla karşılaşırken, Stoacı felsefenin öğretilerine sıkı sıkıya bağlı kalır. Meditations, onun içsel düşüncelerini ve ruhsal mücadelelerini yansıtırken, aynı zamanda insanın doğası, adalet, erdem ve kader gibi evrensel konuları da ele alır.",
      'metamorphosis.jpg':
          "Roman, Gregor Samsa adında bir adamın beklenmedik bir dönüşüm geçirmesiyle başlar. Bir sabah uyandığında, Gregor kendini bir böceğe dönüşmüş olarak bulur. Bu dönüşüm, hem fiziksel hem de psikolojik bir yabancılaşma ve izolasyon sürecini başlatır.",
      'Les Miserables.jpg':
          "Valjean, haksız yere on dokuz yıl hapis cezasına çarptırılmıştır ve bu cezadan sonra topluma yeniden entegre olmaya çalışırken, ahlaki bir yolculuğa çıkar. Onunla birlikte, polis memuru Javert gibi diğer karakterler de yer alır.",
      'Middlemarch.jpg':
          "Hikaye, genç ve idealist Dorothea Brooke'un evlilikle ilgili hayalleriyle başlar. Dorothea, zengin ancak yaşlı bir din adamı olan Bay Casaubon'a evlenir, ancak evlilik hayallerinin tam tersi bir hayatla karşılaşır.",
      'pickwick club.jpg':
          "Roman, Samuel Pickwick adında zengin ve neşeli bir adamın liderliğinde bir araya gelen bir grup kişiden oluşan Pickwick Kulübü'nün maceralarını takip eder. ",
      'mor salkimli ev.jpg':
          "Mor Salkımlı Ev, Osmanlı İmparatorluğu'nun son dönemlerinde, İstanbul'da yaşayan bir ailenin hikayesini anlatır. Romanın baş karakteri Feride, modernleşme ve gelenek arasında sıkışıp kalan bir genç kadındır. ",
      'othello.jpg':
          "Othello, William Shakespeare tarafından yazılmış ünlü bir trajedi oyunudur. Oyun, kıskançlık, ihanet ve ırkçılık gibi temaları işler ve dramatik bir şekilde bir asker olan Othello'nun trajik düşüşünü anlatır.",
      'odysey.jpg':
          "Odyssey, sadakat, aşk, cesaret, onur, kurnazlık ve adalet gibi evrensel temaları işler. Ayrıca, Homeros'un destanı, Antik Yunan mitolojisinin, törelerin ve değerlerin zengin bir portresini sunar.",
      'peter pan.jpg':
          "Peter Pan, J.M. Barrie tarafından yazılan ünlü bir çocuk kitabıdır. Hikaye, Peter Pan adındaki bir çocuk ile Wendy, John ve Michael adındaki üç kardeşin fantastik maceralarını anlatır.",
      'under fire.jpg':
          "Roman, Fransız bir taburunun bir yıllık cephede geçen zamanını ve askerlerin günlük yaşamını, savaşın korkunç gerçekleriyle yüzleşmelerini anlatır.",
      'wanders.jpg':
          "Roman, Ove'nin Norveç kırsalında gezinirken yaşadığı maceraları ve karşılaştığı çeşitli karakterleri anlatır. Ove, toplumun normlarına uymayan biridir ve geleneksel yaşam biçimlerine karşı çıkar. ",
      'peter pan in kensington gardens.jpg':
          "Hikaye, Peter Pan'in bebekken kaybolmasıyla başlar ve Kensington Bahçeleri'nde yaşadığı fantastik maceralarla devam eder. Peter, bahçelerdeki diğer çocuklarla tanışır, hayvanlarla konuşur ve tılsımlı bir dünyanın içine çekilir. ",
      'sherlock holmes.jpg':
          "Holmes, zeki ve dikkatli bir dedektiftir ve olayları çözmek için gözlemlerini ve mantığını kullanır. Serinin birçok hikayesinde, Holmes ve Watson, Londra'nın sokaklarında ve dönemin diğer yerlerinde, cinayetler, hırsızlıklar, kayıp kişiler ve gizemli olaylarla karşı karşıya gelirler.",
      'robinson crouse.jpg':
          "Roman, bir gemi kazasından sonra uzak bir adaya düşen ve orada yalnız başına hayatta kalmaya çalışan İngiliz denizci Robinson Crusoe'nun hikayesini anlatır. Crusoe, adada yıllar geçirirken hayatta kalmak için mücadele eder, barınaklar inşa eder, yiyecek bulur ve kendisini adada yalnız hissetmemek için zamanını çeşitli faaliyetlerle doldurur.",
      'ruh adam.jpg':
          " Romanda hayatını askerliğe adamış bir adamın askerlik mesleğinden atıldıktan sonra yaşadığı ruhsal durumu, bir aşk çıkmazı içine düşmesi, aklı ve kalbi arasında kalmasından dolayı kendi nefsi ile olan mücadelesi anlatılır.",
      'aydakı kadın.jpg':
          "Leyla çocuksuz evli bir kadındır. Once Suat'la sevişmiş, sonra izah edilemeyecek bir ruh hali ile Refik'le evlenmiştir.",
      'emma.jpg':
          "Emma, gençlik gururu ve romantik yanlış anlamalar hakkında bir romandır. Roman, kurgusal bir köy olan Highbury'de ve köyün etrafındaki mülkler Hartfield, Randalls ve Donwell Abbey'de geçmekte ve birkaç aile arasındaki ilişkileri konu almaktadır.",
      'kurk mantolu madonna.jpg':
          "Raif Efendi, içine kapanık, melankolik ve dış dünyaya uyum sağlamakta zorlanan bir karakterdir. Hayatı boyunca pek çok zorluğa katlanmış ve haksızlıklara karşı gelmekte başarısız olmuştur. Sevmediği bir kadınla evlenmiş ve çocukları olmuştur, ancak kendi isteklerine uygun bir hayat yaşayamamıştır. Gerçekten hissettiği tek mutlu anı günlüğüne yazmıştır.",
      'Love and Mr Lewisham.jpg':
          "Romanın başında Bay Lewisham, Sussex'teki bir erkek okulunda 18 yaşında bir öğretmendir ve yılda kırk pound kazanmaktadır. Akrabalarını ziyarete gelen Ethel Henderson ile tanışır ve ona aşık olur",
    },
  };
}
